...
std::string_view color_override_name(ColorOverride value) {
  switch (value) {
    case ColorOverride::automatic:
      return "automatic";
    case ColorOverride::bt601_limited:
      return "bt601_limited";
    case ColorOverride::bt601_full:
      return "bt601_full";
    case ColorOverride::bt709_limited:
      return "bt709_limited";
    case ColorOverride::bt709_full:
      return "bt709_full";
  }
  return "automatic";
}
...
Config parse_config(const json& root) {
  ...
  result.capture_mode.frame_rate.numerator =
      rate.at("numerator").get<std::int32_t>();
  result.capture_mode.frame_rate.denominator =
      rate.at("denominator").get<std::int32_t>();
  ...
  result.mouse_mode =
      parse_mouse_mode(control.at("mouse_mode").get<std::string>());
  result.target_aspect = parse_target_aspect(
      control.value("target_aspect", std::string("full_frame")));
  ...
  if (result.window.width < 960 || result.window.height < 640 ||
    ...
    throw std::runtime_error(
        "configuration values are outside supported bounds");
  }
  return result;
}
                ...
                {"frame_rate",
                 {
                     ...
                     {"denominator",
                      config.capture_mode.frame_rate.denominator},
                 }},
                {"device_format",
                 pixel_format_name(config.capture_mode.device_format)},
                {"delivered_format",
                 pixel_format_name(config.capture_mode.delivered_format)},
...
void save_config(const std::filesystem::path& path, const Config& config) {
  ...
  try {
    ...
    if (!output)
      throw std::runtime_error("could not open temporary configuration file");
