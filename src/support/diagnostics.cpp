...
using TimeWindow =
    SlidingWindow<Diagnostics::Clock::time_point, kWindowCapacity>;
...
double fps(const TimeWindow& window) noexcept {
  ...
  return seconds > 0.0 ? static_cast<double>(window.size() - 1U) / seconds
                       : 0.0;
}
...
LatencySnapshot latency_snapshot(const DurationWindow& window) {
  ...
  for (std::size_t i = 0; i < window.size(); ++i) {
    milliseconds[i] =
        std::chrono::duration<double, std::milli>(window.ordered(i)).count();
  ...
  std::nth_element(
      milliseconds.begin(), milliseconds.begin() + (rank - 1U),
...
void Diagnostics::record_sample_to_gpu_submit(
    const Duration duration) noexcept {
  std::lock_guard lock(impl_->mutex);
  impl_->sample_to_gpu_submit.push(nonnegative(duration));
}
void Diagnostics::record_sdl_to_control_dequeue(
    const Duration duration) noexcept {
  std::lock_guard lock(impl_->mutex);
...
void Diagnostics::set_mailbox_overwrites(
    const std::uint64_t sample_mailbox,
...
DiagnosticsSnapshot Diagnostics::snapshot() const {
  ...
  result.sdl_to_control_dequeue =
      latency_snapshot(impl_->sdl_to_control_dequeue);
...
std::shared_ptr<spdlog::logger> initialize_logging(
