...
CodecResult failure(std::string message) { return {CodecStatus::failed, std::move(message)}; }
...
// Validate the framing and recovery boundary before passing untrusted bytes to FFmpeg.
...
public:
    ...
    CodecDiagnostic diagnostic() const override {
        ...
        value.hardware_verified=hardware_active_ || (context_ && backend_==CodecBackend::ffmpeg_software);
    ...
    CodecResult configure(const CodecConfig& config) override {
        ...
        context_->max_pixels=static_cast<std::int64_t>(kMaxCaptureWidth)*kMaxCaptureHeight;
        context_->err_recognition=AV_EF_EXPLODE;
        context_->opaque=this;
        context_->get_format=[](AVCodecContext* ctx, const AVPixelFormat* formats) {
            ...
            return AV_PIX_FMT_NONE; // Explicit hardware never falls back to software.
        };
        if (backend_==CodecBackend::videotoolbox) {
            ...
            if (result<0) { shutdown(); return {CodecStatus::unsupported, "VideoToolbox device: "+av_error(result)}; }
            context_->hwaccel_flags=0;
        }
        const int result=avcodec_open2(context_, codec, nullptr);
        if (result<0) { shutdown(); return failure("open HEVC: "+av_error(result)); }
        diagnostic_=backend_==CodecBackend::videotoolbox ?
            "requested=videotoolbox output=unverified apple_session_hardware=unverified" :
    ...
    CodecResult submit(const EncodedAccessUnit& au) override {
        ...
        if (result>=0) {
            ...
            packet->pts=next_token_; packet->dts=next_token_;
            result=avcodec_send_packet(context_, packet);
        }
        av_packet_free(&packet);
        if (result==AVERROR(EAGAIN)) return {CodecStatus::again, "poll decoder then retry the same AU"};
        if (result<0) { failed_=true; return failure("send HEVC: "+av_error(result)); }
    ...
    CodecResult poll(VideoFrame& output) override {
        ...
        if (result<0) { failed_=true; return failure("receive HEVC: "+av_error(result)); }
        const auto it=pending_.find(frame->pts);
        if (it==pending_.end() || frame->width!=static_cast<int>(config_.width) ||
            frame->height!=static_cast<int>(config_.height) || (frame->flags&AV_FRAME_FLAG_CORRUPT)) {
            failed_=true; return failure("HEVC output metadata, dimensions or integrity mismatch");
        }
        const auto meta=it->second;
        if (backend_==CodecBackend::videotoolbox) {
            if (frame->format!=AV_PIX_FMT_VIDEOTOOLBOX || !frame->hw_frames_ctx) {
                failed_=true; return failure("VideoToolbox did not produce hardware-context frames");
            }
            std::string hardware="unverified";
#if KVMUX_HAS_VIDEOTOOLBOX
            // FFmpeg may keep its session private. Do not infer this property from pixel format.
            const auto* vt=static_cast<const AVVideotoolboxContext*>(context_->hwaccel_context);
            if (vt && vt->session) {
                CFTypeRef value=nullptr;
                if (VTSessionCopyProperty(vt->session, kVTDecompressionPropertyKey_UsingHardwareAcceleratedVideoDecoder,
                                          kCFAllocatorDefault, &value)==noErr && value) {
                    if (CFGetTypeID(value)==CFBooleanGetTypeID())
                        hardware=CFBooleanGetValue(static_cast<CFBooleanRef>(value)) ? "yes" : "no";
                    CFRelease(value);
                }
            }
#endif
            if (hardware=="no") { failed_=true; return failure("VideoToolbox reports a software decoding session"); }
            auto cpu=own(av_frame_alloc());
            if (!cpu) { failed_=true; return failure("allocate HEVC readback"); }
            int transferred=av_hwframe_transfer_data(cpu.get(), frame.get(), 0);
            if (transferred>=0) transferred=av_frame_copy_props(cpu.get(), frame.get());
            if (transferred<0) { failed_=true; return failure("VideoToolbox CPU transfer: "+av_error(transferred)); }
            diagnostic_="requested=videotoolbox output=videotoolbox_vld hw_frames_ctx=yes cpu_transfer="+
                std::string(av_get_pix_fmt_name(static_cast<AVPixelFormat>(cpu->format)))+" apple_session_hardware="+hardware;
            hardware_active_=hardware=="yes";
            frame=std::move(cpu);
        } else {
            const auto* desc=av_pix_fmt_desc_get(static_cast<AVPixelFormat>(frame->format));
            if (!desc || (desc->flags&AV_PIX_FMT_FLAG_HWACCEL) || frame->hw_frames_ctx) {
                failed_=true; return failure("software decoder returned non-CPU frame");
            }
            diagnostic_="requested=ffmpeg_software output="+std::string(desc->name);
        }
        pending_.erase(it);
        frame->pts=meta.pts;
        ...
        if (meta.transfer!=AVCOL_TRC_UNSPECIFIED) frame->color_trc=meta.transfer;
        if (meta.sar.num>0 && meta.sar.den>0) frame->sample_aspect_ratio=meta.sar;
        ...
        if (frame->colorspace==AVCOL_SPC_BT470BG || frame->colorspace==AVCOL_SPC_SMPTE170M) matrix=ColorMatrix::bt601;
        output={meta.generation, meta.sequence, meta.arrival, std::chrono::steady_clock::now(),
            meta.pts, 1, 1'000'000'000, static_cast<std::uint32_t>(sar.num>0 ? sar.num : 1),
            static_cast<std::uint32_t>(sar.den>0 ? sar.den : 1), frame_color_range(*frame), matrix, std::move(frame)};
        return {};
    }
private:
    struct Metadata {
...
std::unique_ptr<VideoDecoder> create_ffmpeg_decoder(CodecBackend backend, std::string& error) {
    error.clear();
    if (backend!=CodecBackend::videotoolbox && backend!=CodecBackend::ffmpeg_software) {
        error="FFmpeg decoder requires explicit videotoolbox or ffmpeg_software backend";
        return {};
    }
#if !KVMUX_HAS_VIDEOTOOLBOX
