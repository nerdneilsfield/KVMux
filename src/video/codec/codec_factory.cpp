...
// Never replace a live reference chain; the caller owns recovery and
// generation.
...
class AutomaticCodec : public Interface {
 public:
  ...
  AutomaticCodec(Factory factory, CodecBackend hardware)
      : factory_(factory), hardware_(hardware) {}
  ...
  CodecResult submit(const Input& input) override {
    ...
    if (first_submit_ && selected_->backend() == hardware_ &&
        (result.status == CodecStatus::failed ||
         result.status == CodecStatus::unsupported)) {
  ...
  CodecResult finish() override {
    return selected_ ? selected_->finish() : unavailable();
  }
 ...
 protected:
  CodecResult unavailable() const {
    return {CodecStatus::failed, "Automatic codec is not configured"};
  }
...
class AutomaticEncoder final
    : public AutomaticCodec<VideoEncoder, EncoderInput, EncodedAccessUnit> {
 public:
  AutomaticEncoder()
      : AutomaticCodec(create_video_encoder, CodecBackend::jetson_gstreamer) {}
  CodecResult request_keyframe() override {
    return selected_ ? selected_->request_keyframe() : unavailable();
  }
};
using AutomaticDecoder =
    AutomaticCodec<VideoDecoder, EncodedAccessUnit, VideoFrame>;
}  // namespace

std::unique_ptr<VideoEncoder> create_video_encoder(CodecBackend backend,
                                                   std::string& error) {
  error.clear();
  if (backend == CodecBackend::automatic)
    return std::make_unique<AutomaticEncoder>();
  if (backend == CodecBackend::ffmpeg_software)
    return create_ffmpeg_encoder(backend, error);
  if (backend == CodecBackend::jetson_gstreamer) {
#if KVMUX_HAS_JETSON_ENCODER
    return create_jetson_encoder(error);
#else
    error =
        "Jetson encoder unavailable: build with KVMUX_JETSON_ENCODER=ON on "
        "Linux with GStreamer core/app/video development packages";
    return {};
#endif
  }
...
std::unique_ptr<VideoDecoder> create_video_decoder(CodecBackend backend,
                                                   std::string& error) {
  error.clear();
  if (backend == CodecBackend::automatic)
    return std::make_unique<AutomaticDecoder>(create_video_decoder,
...
#else
    error =
        "VideoToolbox decoder unavailable: build with KVMUX_VIDEOTOOLBOX=ON on "
        "macOS";
    return {};
#endif
  }
