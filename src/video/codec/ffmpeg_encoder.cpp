...
bool valid_plane(AVFrame* frame, int plane, int width, int height) {
    ...
    if (!buffer || !frame->data[plane] || frame->linesize[plane]<width) return false;
    ...
    const auto bytes=static_cast<std::uint64_t>(height-1)*frame->linesize[plane]+width;
...
public:
    ...
    CodecResult configure(const CodecConfig& config) override {
        ...
        if (!config.width || !config.height || config.width>kMaxCaptureWidth ||
            ...
            config.fps_numerator<config.fps_denominator ||
            config.fps_numerator>static_cast<unsigned>(std::numeric_limits<int>::max()) ||
            config.fps_denominator>static_cast<unsigned>(std::numeric_limits<int>::max()) ||
    ...
    CodecResult submit(const EncoderInput& input) override {
        if (!context_ || failed_ || finishing_) return fail("encoder needs configure/reset before submit");
        auto* source=input.frame.get();
        if (!source || source->hw_frames_ctx || source->width!=static_cast<int>(config_.width) ||
            source->height!=static_cast<int>(config_.height) || input.generation!=config_.generation ||
            input.pts_ns<0 || input.pts_ns<=last_pts_ ||
            (source->format!=AV_PIX_FMT_NV12 && source->format!=AV_PIX_FMT_YUV420P))
            return {CodecStatus::invalid_input, "Expected matching owned CPU NV12/YUV420P and increasing nonnegative PTS"};
        const int w=source->width, h=source->height;
        const bool planar=source->format==AV_PIX_FMT_YUV420P;
        if (!valid_plane(source,0,w,h) || !valid_plane(source,1,planar?w/2:w,h/2) ||
            (planar && !valid_plane(source,2,w/2,h/2)))
            return {CodecStatus::invalid_input, "Invalid owned CPU frame planes or strides"};
        if (pending_.size()>=4) return {CodecStatus::again, "poll encoder then retry input"};
        ...
        if (result<0) return fail("allocate/copy encoder frame: "+av_error(result));
        for (int y=0;y<h;++y)
            std::memcpy(frame->data[0]+y*frame->linesize[0],source->data[0]+y*source->linesize[0],w);
        for (int y=0;y<h/2;++y) for (int x=0;x<w/2;++x) {
            frame->data[1][y*frame->linesize[1]+x]=source->data[1][y*source->linesize[1]+(planar?x:2*x)];
            frame->data[2][y*frame->linesize[2]+x]=planar?source->data[2][y*source->linesize[2]+x]:source->data[1][y*source->linesize[1]+2*x+1];
        }
        ...
        meta.generation=input.generation; meta.arrival=input.arrival;
        meta.color_range=source->color_range; meta.color_space=source->colorspace;
        meta.color_primaries=source->color_primaries; meta.color_transfer=source->color_trc;
        meta.sample_aspect_ratio=source->sample_aspect_ratio.num>0 && source->sample_aspect_ratio.den>0
            ? source->sample_aspect_ratio : AVRational{1,1};
        pending_.emplace(next_token_++,std::move(meta));
        last_pts_=input.pts_ns; force_idr_=false;
        return {};
    }
    CodecResult poll(EncodedAccessUnit& output) override {
        if (!context_ || failed_) return fail("encoder is not configured or needs reset");
        ...
        if (result<0) {
            ...
            if (result==AVERROR_EOF && pending_.empty()) return {CodecStatus::end_of_stream,{}};
            failed_=true; return fail("receive libx265 packet or missing accepted frame: "+av_error(result));
        }
    ...
    CodecResult request_keyframe() override {
        if (!context_ || failed_ || finishing_) return fail("encoder not accepting keyframe requests");
        force_idr_=true; return {};
    }
    CodecResult finish() override {
        ...
        if (result<0) { failed_=true; return fail("finish libx265: "+av_error(result)); }
        finishing_=true; return {};
    }
    CodecResult reset() override {
        if (!context_) return fail("encoder is not configured");
        const auto config=config_; const auto sequence=encoded_sequence_;
        auto result=configure(config); encoded_sequence_=sequence; return result;
    }
    void shutdown() noexcept override {
        avcodec_free_context(&context_); pending_.clear();
        next_token_=0; last_pts_=-1; force_idr_=true;
        finishing_=false; failed_=false; observed_=false;
    }
private:
    CodecConfig config_{};
...
std::unique_ptr<VideoEncoder> create_ffmpeg_encoder(CodecBackend backend, std::string& error) {
    error.clear();
