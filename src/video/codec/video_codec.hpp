...
[[nodiscard]] constexpr EncodingProfile encoding_profile(
  ...
  return priority == EncodingPriority::quality
             ...
             : EncodingProfile{
                   std::max<std::uint32_t>(250'000, requested_bitrate / 2),
                   "slow", 4};
}
...
struct EncoderInput {
  // Owned CPU frame. Do not mutate while submit executes. Backends copy or
  // retain an AVBufferRef before returning; the caller may release its frame
  // afterwards.
  AvFramePtr frame;
  std::int64_t pts_ns{};
...
struct EncodedAccessUnit {
  ...
  std::uint64_t capture_sequence{}, generation{};
  std::chrono::steady_clock::time_point arrival{};
  // Consecutive encoder outputs; configure starts at 1, reset preserves
  // counter.
...
// Single owner thread. submit/poll are bounded and never wait for codec
// progress. again: input was NOT accepted, or no output is ready. All other
// non-ok results require caller recovery; accepted compressed AUs must never be
// silently dropped. configure/reset/shutdown are lifecycle operations and may
// wait. reset discards pending work, retains configuration, and starts a fresh
// reference chain.
class VideoEncoder {
 public:
