...
class JetsonEncoder final : public VideoEncoder {
 public:
  ...
  CodecBackend backend() const noexcept override {
    return CodecBackend::jetson_gstreamer;
  }
  CodecDiagnostic diagnostic() const override {
    const auto name =
        config_.codec == VideoCodec::h264 ? "nvv4l2h264enc" : "nvv4l2h265enc";
    return {backend(), hardware_active_, hardware_active_,
            std::string("NVIDIA ") + name + " priority=" +
                (config_.priority == EncodingPriority::quality ? "quality"
                                                               : "size") +
                " effective_bitrate=" + std::to_string(effective_bitrate_) +
                (hardware_active_ ? " hardware output verified"
                                  : " output unverified")};
  }
  CodecResult configure(const CodecConfig& config) override {
    ...
    if (!config.width || !config.height || config.width > kMaxCaptureWidth ||
        ...
        (config.height % 2) || !config.fps_numerator ||
    ...
    const auto encoder_name =
        config.codec == VideoCodec::h264 ? "nvv4l2h264enc" : "nvv4l2h265enc";
    const auto output_caps =
        config.codec == VideoCodec::h264 ? "video/x-h264" : "video/x-h265";
    const auto text =
        std::string(
            "appsrc name=input is-live=true format=time block=false "
            "max-buffers=4 max-bytes=0 max-time=0 ! ") +
        "nvvidconv ! video/x-raw(memory:NVMM),format=NV12 ! " + encoder_name +
        " name=encoder num-B-Frames=0 insert-sps-pps=true insert-aud=true "
        "control-rate=1 preset-level=" +
        std::to_string(profile.jetson_preset_level) +
        " bitrate=" + std::to_string(effective_bitrate_) +
        " idrinterval=" + std::to_string(config.keyframe_interval) +
        " iframeinterval=" + std::to_string(config.keyframe_interval) + " ! " +
        output_caps + ",stream-format=byte-stream,alignment=au ! " +
        "appsink name=output sync=false max-buffers=4 drop=false "
        "wait-on-eos=false";
    GError* error = nullptr;
    pipeline_ = gst_parse_launch(text.c_str(), &error);
    if (error || !pipeline_) {
      auto result =
          fail(error ? error->message : "Cannot create Jetson H.26x pipeline");
    ...
    if (gst_element_set_state(pipeline_, GST_STATE_PLAYING) ==
      ...
      shutdown();
      return result.ok() ? fail("Jetson H.26x hardware failed to start")
                         : result;
    }
    return {};
  }
  CodecResult submit(const EncoderInput& input) override {
    ...
    if (!frame || frame->width != static_cast<int>(config_.width) ||
        ...
        (last_pts_ >= 0 && input.pts_ns <= last_pts_) ||
        (frame->format != AV_PIX_FMT_NV12 &&
         frame->format != AV_PIX_FMT_YUV420P))
      return {CodecStatus::invalid_input,
              "Expected matching CPU NV12/YUV420P frame and increasing "
              "nonnegative PTS"};
    ...
    if (frame->hw_frames_ctx || !frame->data[0] || !frame->data[1] ||
        ...
        frame->linesize[1] < (planar ? width / 2 : width) ||
        (planar && (!frame->data[2] || frame->linesize[2] < width / 2)))
      return {CodecStatus::invalid_input,
              "Invalid CPU frame planes or strides"};
    GstVideoInfo info;
    gst_video_info_set_format(&info, GST_VIDEO_FORMAT_NV12, unsigned_width,
                              unsigned_height);
    ...
    for (int y = 0; y < height / 2; ++y) {
      ...
      if (!planar)
        std::memcpy(row, frame->data[1] + y * frame->linesize[1], row_bytes);
      else
    ...
    GST_BUFFER_DURATION(buffer) = gst_util_uint64_scale(
        GST_SECOND, config_.fps_denominator, config_.fps_numerator);
    EncodedAccessUnit metadata;
    metadata.width = config_.width;
    metadata.height = config_.height;
    metadata.pts_ns = input.pts_ns;
    metadata.capture_sequence = input.capture_sequence;
    metadata.generation = input.generation;
    metadata.arrival = input.arrival;
    metadata.color_range = frame->color_range;
    metadata.color_space = frame->colorspace;
    metadata.color_primaries = frame->color_primaries;
    metadata.color_transfer = frame->color_trc;
    metadata.sample_aspect_ratio =
        frame->sample_aspect_ratio.num > 0 && frame->sample_aspect_ratio.den > 0
            ? frame->sample_aspect_ratio
            : AVRational{1, 1};
    const auto flow = gst_app_src_push_buffer(GST_APP_SRC(input_),
                                              buffer);  // takes ownership
    if (flow != GST_FLOW_OK)
      return fail("Jetson appsrc rejected input: " + std::to_string(flow));
  ...
  CodecResult poll(EncodedAccessUnit& output) override {
    ...
    if (!sample) {
      if (finished_ && gst_app_sink_is_eos(GST_APP_SINK(output_)))
        return pending_.empty() ? CodecResult{CodecStatus::end_of_stream, {}}
                                : fail("Encoder EOS lost pending frames");
      return {CodecStatus::again, {}};
    }
    GstBuffer* buffer = gst_sample_get_buffer(sample);
    GstCaps* caps = gst_sample_get_caps(sample);
    const GstStructure* structure =
        caps ? gst_caps_get_structure(caps, 0) : nullptr;
    const char* alignment =
        structure ? gst_structure_get_string(structure, "alignment") : nullptr;
    const char* stream =
        structure ? gst_structure_get_string(structure, "stream-format")
                  : nullptr;
    GstMapInfo map{};
    if (!buffer || !alignment || std::strcmp(alignment, "au") || !stream ||
        ...
        GST_BUFFER_PTS(buffer) !=
            static_cast<GstClockTime>(pending_.front().pts_ns) ||
    ...
    if (keyframe_pending_) {
      ...
      if (!output.idr && !finished_)
        g_signal_emit_by_name(encoder_, "force-IDR");
    }
    return {};
  }
  CodecResult request_keyframe() override {
    if (!encoder_ || finished_)
      return fail("Encoder is not accepting keyframe requests");
    if (auto error = bus_error(); !error.ok()) return error;
    if (!hardware_active_) {
 ...
 private:
  CodecResult bus_error() {
    ...
    auto result = fail(std::string("Jetson H.26x hardware: ") +
...
std::unique_ptr<VideoEncoder> create_jetson_encoder(std::string& error) {
  ...
  if (!gst_init_check(nullptr, nullptr, &init_error)) {
    error =
        init_error ? init_error->message : "GStreamer initialization failed";
  ...
  for (const char* name : {"appsrc", "nvvidconv", "appsink"}) {
    ...
    if (!factory) {
      error =
          std::string("Jetson hardware encoder unavailable: missing ") + name;
      return nullptr;
    }
