...
bool plane_fits(const PlaneLayout& plane, const std::size_t payload_size) {
  ...
  const auto stride =
      static_cast<std::size_t>(plane.stride < 0 ? -plane.stride : plane.stride);
  ...
  return plane.offset <= payload_size &&
         row_distance <= payload_size - plane.offset &&
...
std::optional<CaptureSample> CaptureSample::make_raw(
  ...
  const std::size_t expected_planes =
  ...
  sample.raw =
      RawPayload{format, std::vector<PlaneLayout>(planes.begin(), planes.end()),
...
std::optional<CaptureSample> CaptureSample::make_mjpeg(
  ...
  if (bytes.size() >
      std::numeric_limits<std::size_t>::max() - kInputPaddingBytes) {
    return std::nullopt;
  }
