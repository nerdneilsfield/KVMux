...
VideoFrame metadata(const CaptureSample& sample, AvFramePtr frame) {
  ...
  return {sample.generation,
          ...
          sample.device_time_base_numerator,
...
std::optional<VideoFrame> VideoProcessor::process(const CaptureSample& sample) {
  ...
  if (!valid_dimensions(sample.width, sample.height) ||
      (static_cast<int>(sample.raw.has_value()) +
           static_cast<int>(sample.mjpeg.has_value()) +
           static_cast<int>(bool(sample.decoded)) !=
       1)) {
  ...
  if (sample.decoded) {
    ...
    if (!description || (description->flags & AV_PIX_FMT_FLAG_HWACCEL) ||
        planes < 1 || planes > 4 ||
        av_image_fill_linesizes(minimum.data(), format, frame.width) < 0) {
      fail("decoded frame must have a supported CPU layout");
      return std::nullopt;
    }
    for (int plane = 0; plane < planes; ++plane) {
      const auto* buffer =
          av_frame_get_plane_buffer(sample.decoded.get(), plane);
      const int rows =
          plane == 1 || plane == 2
              ? AV_CEIL_RSHIFT(frame.height, description->log2_chroma_h)
              : frame.height;
      const auto size =
          frame.linesize[plane] > 0
              ? static_cast<std::size_t>(rows - 1) * frame.linesize[plane] +
                    minimum[plane]
              : 0;
      if (!buffer || !frame.data[plane] ||
          frame.linesize[plane] < minimum[plane] ||
          frame.data[plane] < buffer->data || size > buffer->size ||
          static_cast<std::size_t>(frame.data[plane] - buffer->data) >
              buffer->size - size) {
...
std::optional<VideoFrame> VideoProcessor::process_raw(
    const CaptureSample& sample) {
  const auto& raw = *sample.raw;
  const auto format = av_format(raw.format);
  const int expected_planes =
      raw.format == PixelFormat::nv12
          ? 2
          : (raw.format == PixelFormat::yuv420p ||
                     raw.format == PixelFormat::yuv422p ||
                     raw.format == PixelFormat::yuv444p
                 ? 3
                 : 1);
  if (format == AV_PIX_FMT_NONE ||
      raw.planes.size() != static_cast<std::size_t>(expected_planes)) {
  ...
  for (int plane_index = 0; plane_index < expected_planes; ++plane_index) {
    ...
    for (std::size_t row = 0; row < plane.rows; ++row) {
      const auto source_offset =
          static_cast<std::ptrdiff_t>(row) * plane.stride;
      auto* destination =
          frame->data[plane_index] +
...
std::optional<VideoFrame> VideoProcessor::process_mjpeg(
    const CaptureSample& sample) {
  if (!ensure_mjpeg_decoder()) {
    return std::nullopt;
  }
...
void VideoProcessor::fail(std::string message) {
