...
static Rational frame_rate_from_duration(CMTime duration) {
    if (!CMTIME_IS_NUMERIC(duration) || duration.value <= 0 || duration.timescale <= 0)
        return {};
    ...
    return {static_cast<std::int32_t>(numerator),
            static_cast<std::int32_t>(denominator)};
}

static std::string fourcc_name(OSType code) {
    char text[5] = {static_cast<char>((code >> 24) & 0xff),
...
static ColorMatrix color_matrix(CVImageBufferRef image) {
    ...
    const ColorMatrix result = value == kCVImageBufferYCbCrMatrix_ITU_R_709_2
                                   ? ColorMatrix::bt709
...
- (void)captureOutput:(AVCaptureOutput*)output
    ...
    @autoreleasepool {
        ...
        if (image == nullptr || CVPixelBufferLockBaseAddress(
        ...
        do {
            ...
            if (width > std::numeric_limits<std::uint32_t>::max() ||
                ...
                !valid_dimensions(static_cast<std::uint32_t>(width),
            ...
            for (std::size_t plane = 0; plane < count; ++plane) {
                ...
                const std::size_t rows = planar ? CVPixelBufferGetHeightOfPlane(image, plane)
                                                : height;
                const std::size_t stride = planar ? CVPixelBufferGetBytesPerRowOfPlane(image, plane)
                                                  : CVPixelBufferGetBytesPerRow(image);
                std::size_t row_bytes = 0;
                if (format == PixelFormat::nv12) row_bytes = width;
                else if (format == PixelFormat::uyvy && width <= SIZE_MAX / 2) row_bytes = width * 2;
                else if (format == PixelFormat::bgra && width <= SIZE_MAX / 4) row_bytes = width * 4;
                if (base == nullptr || row_bytes == 0 || stride < row_bytes ||
                    rows == 0 || stride > kMaxRawSampleBytes / rows ||
            ...
            result = CaptureSample::make_raw(
                _generation, 0, std::chrono::steady_clock::now(),
            ...
            result->color_range =
                CVPixelBufferGetPixelFormatType(image) ==
                    ...
                    ? ColorRange::full
                    : (format == PixelFormat::nv12 || format == PixelFormat::uyvy
                           ? ColorRange::limited
                           : ColorRange::unknown);
...
static void set_failure(const std::shared_ptr<AvfState>& state,
                        std::uint64_t generation, CaptureState status,
                        std::string error) {
...
static void tear_down(const std::shared_ptr<AvfState>& state,
...
static void configure(const std::shared_ptr<AvfState>& state, CaptureMode requested,
    ...
    for (AVCaptureDeviceFormat* format in device.formats) {
        ...
        for (AVFrameRateRange* range in format.videoSupportedFrameRateRanges) {
            if (CMTIME_IS_NUMERIC(candidate) &&
                CMTimeCompare(candidate, range.minFrameDuration) >= 0 &&
    ...
    KVMuxVideoDelegate* delegate =
        [[KVMuxVideoDelegate alloc] initWithState:state generation:generation];
    ...
    actual.device_format = pixel_format(
        CMFormatDescriptionGetMediaSubType(selected.formatDescription));
    actual.delivered_format = pixel_format(chosen);
    actual.device_format_name = fourcc_name(
        CMFormatDescriptionGetMediaSubType(selected.formatDescription));
    std::lock_guard lock(state->mutex);
...
public:
    ...
    ~AvfoundationCaptureSource() override {
        ...
        const auto work = ^{ tear_down(state, callback_queue); };
        if (dispatch_get_specific(this) == this) work();
        else dispatch_sync(session_queue_, work);
    }

    std::vector<DeviceInfo> enumerate_devices() override {
        std::vector<DeviceInfo> result;
        for (AVCaptureDevice* device in video_devices()) {
            result.push_back({CaptureBackend::avfoundation, to_utf8(device.uniqueID),
                              to_utf8(device.localizedName), std::nullopt,
                              std::nullopt, {}, false});
        }
        return result;
    }
    ...
    std::vector<CaptureMode> enumerate_modes(const std::string& stable_id) override {
        ...
        for (AVCaptureDeviceFormat* format in device.formats) {
            ...
            if (size.width <= 0 || size.height <= 0 ||
                ...
                continue;
            const std::string name = fourcc_name(
                CMFormatDescriptionGetMediaSubType(format.formatDescription));
            for (AVFrameRateRange* range in format.videoSupportedFrameRateRanges) {
                Rational fps = frame_rate_from_duration(range.minFrameDuration);
                if (fps.numerator <= 0) continue;
                const PixelFormat native = pixel_format(
                    CMFormatDescriptionGetMediaSubType(format.formatDescription));
                CaptureMode mode{stable_id, static_cast<std::uint32_t>(size.width),
                                 static_cast<std::uint32_t>(size.height), fps,
                                 native, PixelFormat::unknown, name};
    ...
    void start(const CaptureMode& mode) override {
        ...
        if (authorization == AVAuthorizationStatusAuthorized) {
            ...
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo
                ...
                if (granted) dispatch_async(session_queue, authorized);
                else set_failure(state, generation, CaptureState::permission_denied,
            ...
            set_failure(state, generation, CaptureState::fault,
