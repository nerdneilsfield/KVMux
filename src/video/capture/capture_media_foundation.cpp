...
std::string hresult_text(const char* operation, HRESULT hr) {
    char value[32]{};
    std::snprintf(value, sizeof(value), "0x%08lX", static_cast<unsigned long>(hr));
...
std::string utf8(const wchar_t* text, UINT32 length) {
    ...
    WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, text,
                        static_cast<int>(length), result.data(), count, nullptr, nullptr);
    return result;
}
...
public:
    ...
    std::vector<CaptureMode> enumerate_modes(const std::string& stable_id) override {
        return invoke([this, stable_id] { return enumerate_modes_on_worker(stable_id); });
    }
    ...
    void stop() noexcept override {
        if (!worker_.joinable() || std::this_thread::get_id() == worker_.get_id()) return;
        try {
...
private:
        ...
        try {
            ...
            for (;;) {
                std::function<void()> command;
                {
                    ...
                    command_cv_.wait(lock, [this] { return quitting_ || !commands_.empty(); });
    ...
    std::vector<ComPtr<IMFActivate>> activate_devices() {
        ...
        UINT32 count = 0;
        check(MFEnumDeviceSources(attributes.Get(), &raw, &count), "MFEnumDeviceSources");
    ...
    std::vector<DeviceInfo> enumerate_devices_on_worker() {
        ...
        for (auto& device : activate_devices()) {
            ...
            result.push_back({CaptureBackend::media_foundation, id,
                              activation_string(device.Get(), MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME),
    ...
    ComPtr<IMFMediaSource> open_source(const std::string& stable_id) {
        for (auto& device : activate_devices()) {
            if (activation_string(device.Get(),
                                  MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK) == stable_id) {
    ...
    ComPtr<IMFSourceReader> make_reader(IMFMediaSource* source,
        ...
        ComPtr<IMFAttributes> attributes;
        check(MFCreateAttributes(&attributes, callback ? 4 : 3), "MFCreateAttributes");
        check(attributes->SetUINT32(MF_READWRITE_DISABLE_CONVERTERS, TRUE),
              "disable Source Reader converters");
        check(attributes->SetUINT32(MF_SOURCE_READER_ENABLE_VIDEO_PROCESSING, FALSE),
              "disable Source Reader video processing");
        check(attributes->SetUINT32(MF_SOURCE_READER_DISCONNECT_MEDIASOURCE_ON_SHUTDOWN, TRUE),
        ...
        check(MFCreateSourceReaderFromMediaSource(source, attributes.Get(), &reader),
    ...
    std::vector<CaptureMode> enumerate_modes_on_worker(const std::string& stable_id) {
    ...
    ComPtr<IMFMediaType> find_native_type(IMFSourceReader* reader,
        ...
        for (DWORD index = 0;; ++index) {
            ...
            if (media_type_mode(type.Get(), requested.device_id, mode) &&
                same_mode(mode, requested)) return type;
        }
        throw std::runtime_error("requested Media Foundation native mode is unavailable");
    }
    ...
    void start_on_worker(const CaptureMode& requested) {
        ...
        check(reader_->SetCurrentMediaType(MF_SOURCE_READER_FIRST_VIDEO_STREAM, nullptr,
                                           native_type.Get()),
              "SetCurrentMediaType");
        CaptureMode actual;
        check(media_type_mode(native_type.Get(), requested.device_id, actual) ? S_OK : E_FAIL,
        ...
        native_type->GetUINT32(MF_MT_YUV_MATRIX, &yuv_matrix);
        {
            ...
            color_range_ = nominal_range == MFNominalRange_0_255 ? ColorRange::full :
                           nominal_range == MFNominalRange_16_235 ? ColorRange::limited :
                                                                   ColorRange::unknown;
            color_matrix_ = yuv_matrix == MFVideoTransferMatrix_BT709 ? ColorMatrix::bt709 :
    ...
    void stop_on_worker() noexcept {
        ...
        std::lock_guard lock(state_mutex_);
        if (snapshot_.state != CaptureState::fault) snapshot_.state = CaptureState::stopped;
    }
    ...
    void request_sample() noexcept {
        ...
        const HRESULT hr = reader_->ReadSample(MF_SOURCE_READER_FIRST_VIDEO_STREAM, 0,
    ...
    void update_current_type() {
        ...
        UINT32 sar_numerator = 1, sar_denominator = 1;
        MFGetAttributeRatio(type.Get(), MF_MT_PIXEL_ASPECT_RATIO,
                            &sar_numerator, &sar_denominator);
        UINT32 nominal_range = MFNominalRange_Unknown;
    ...
    std::optional<CaptureSample> copy_sample(IMFSample* sample, LONGLONG timestamp) {
        CaptureMode mode;
        ...
        if (mode.delivered_format == PixelFormat::mjpeg) {
            ...
            try {
                result = CaptureSample::make_mjpeg(generation, sequence, arrival, mode.width,
                                                    mode.height, std::span<const std::uint8_t>(bytes, length));
            } catch (...) {
                buffer->Unlock();
                throw;
            }
            ...
            case PixelFormat::uyvy: row_bytes = static_cast<std::size_t>(mode.width) * 2U; break;
            case PixelFormat::bgra: row_bytes = static_cast<std::size_t>(mode.width) * 4U; break;
            case PixelFormat::nv12: row_bytes = mode.width; rows += mode.height / 2U; break;
            default: return std::nullopt;
            }
            if (row_bytes == 0 || rows > kMaxRawSampleBytes / row_bytes) return std::nullopt;
            ...
            if (SUCCEEDED(buffer.As(&buffer2d))) {
                ...
                const auto absolute_pitch = static_cast<std::size_t>(pitch < 0 ? -static_cast<std::int64_t>(pitch) : pitch);
                ...
                for (std::size_t row = 0; row < rows; ++row)
                    std::memcpy(owned.data() + row * row_bytes,
                                scanline + static_cast<std::ptrdiff_t>(row) * pitch, row_bytes);
                buffer2d->Unlock2D();
            } else {
                BYTE* bytes = nullptr;
                DWORD maximum = 0, length = 0;
                check(buffer->Lock(&bytes, &maximum, &length), "IMFMediaBuffer::Lock");
                LONG stride = native_stride != 0 ? native_stride : static_cast<LONG>(row_bytes);
                const auto absolute_stride = static_cast<std::size_t>(stride < 0 ? -static_cast<std::int64_t>(stride) : stride);
                if (absolute_stride < row_bytes || rows > 0 &&
                    (rows - 1U) > (length >= row_bytes ? (length - row_bytes) / absolute_stride : 0)) {
                    buffer->Unlock();
                    return std::nullopt;
                }
                ...
                for (std::size_t row = 0; row < rows; ++row)
                    std::memcpy(owned.data() + row * row_bytes,
                                first + static_cast<std::ptrdiff_t>(row) * stride, row_bytes);
                buffer->Unlock();
            }
            std::vector<PlaneLayout> planes;
            if (mode.delivered_format == PixelFormat::nv12) {
                planes.push_back({0, static_cast<std::ptrdiff_t>(row_bytes), row_bytes, mode.height});
            ...
            result = CaptureSample::make_raw(generation, sequence, arrival, mode.width,
                                              mode.height, mode.delivered_format, planes, owned);
        }
        if (result) {
    ...
    void on_read_sample(HRESULT status, DWORD, DWORD flags, LONGLONG timestamp,
        ...
        read_outstanding_.store(false, std::memory_order_release);
        try {
            if (FAILED(status)) throw std::runtime_error(hresult_text("OnReadSample", status));
            if ((flags & MF_SOURCE_READERF_CURRENTMEDIATYPECHANGED) != 0) update_current_type();
            if ((flags & MF_SOURCE_READERF_ERROR) != 0)
                throw std::runtime_error("Media Foundation Source Reader reported an error");
            if (sample != nullptr) {
                auto copied = copy_sample(sample, timestamp);
                if (!copied) throw std::runtime_error("invalid Media Foundation video sample");
                {
                    std::lock_guard lock(sample_mutex_);
                    if (latest_sample_) {
