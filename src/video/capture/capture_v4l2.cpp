...
std::optional<std::uint32_t> fourcc_for(PixelFormat format) {
    const auto it = std::find_if(std::begin(kFormats), std::end(kFormats),
...
bool supported_size(std::uint32_t width, std::uint32_t height) {
    return width > 0 && height > 0 && width <= kMaxCaptureWidth && height <= kMaxCaptureHeight;
}
...
std::vector<std::string> device_paths() {
    ...
    if (fs::is_directory(by_id, ec)) {
        ...
        for (fs::directory_iterator it(by_id, ec), end; !ec && it != end; it.increment(ec)) {
            if (it->is_symlink(ec)) links.push_back(it->path());
        }
        std::sort(links.begin(), links.end());
        for (const auto& link : links) {
            const auto canonical = fs::weakly_canonical(link, ec);
            if (!ec && targets.insert(canonical.string()).second) result.push_back(link.string());
            ec.clear();
        }
    }
    ...
    for (fs::directory_iterator it(dev, ec), end; !ec && it != end; it.increment(ec)) {
    ...
    for (const auto& node : nodes) {
        ...
        if (!ec && targets.insert(canonical.string()).second) result.push_back(node.string());
        ec.clear();
    }
    return result;
}
...
public:
    ...
    std::vector<DeviceInfo> enumerate_devices() override {
        ...
        for (const auto& path : device_paths()) {
            ...
            if (fd < 0) continue;
            v4l2_capability cap{};
            const bool usable = xioctl(fd, VIDIOC_QUERYCAP, &cap) == 0 && capture_capable(cap);
            ::close(fd);
            if (!usable) continue;
            DeviceInfo info;
            ...
            for (desc.index = 0; xioctl(fd, VIDIOC_ENUM_FMT, &desc) == 0; ++desc.index) {
                ...
                for (size.index = 0; xioctl(fd, VIDIOC_ENUM_FRAMESIZES, &size) == 0; ++size.index) {
                    ...
                    if (size.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
                        ...
                        dimensions.emplace_back(std::min(size.stepwise.max_width, kMaxCaptureWidth),
                    ...
                    for (const auto [width, height] : dimensions) {
                        ...
                        for (interval.index = 0; xioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &interval) == 0;
                             ++interval.index) {
                            ...
                            if (!supported_rate(numerator, denominator) ||
                                numerator > static_cast<std::uint32_t>(std::numeric_limits<std::int32_t>::max()) ||
                                denominator > static_cast<std::uint32_t>(std::numeric_limits<std::int32_t>::max())) {
...
private:
    struct Mapping { void* address{MAP_FAILED}; std::size_t length{}; };

    void fail(std::string message, int saved_errno = 0) {
        std::lock_guard lock(mutex_);
        snapshot_.state = (saved_errno == EACCES || saved_errno == EPERM)
                              ? CaptureState::permission_denied : CaptureState::fault;
    ...
    void run(const CaptureMode requested) {
        ...
        if (event_fd < 0) { fail(errno_text("eventfd"), errno); return; }
        stop_fd_.store(event_fd, std::memory_order_release);
        if (stop_requested_.load(std::memory_order_acquire)) {
            cleanup();
            return;
        }
        video_fd = ::open(requested.device_id.c_str(), O_RDWR | O_NONBLOCK | O_CLOEXEC);
        if (video_fd < 0) { const int e = errno; fail(errno_text("open"), e); cleanup(); return; }
        v4l2_capability cap{};
        if (xioctl(video_fd, VIDIOC_QUERYCAP, &cap) != 0 || !capture_capable(cap)) {
            fail(errno_text("VIDIOC_QUERYCAP"), errno); cleanup(); return;
        }
        const auto fourcc = fourcc_for(requested.device_format);
        if (!fourcc || !supported_size(requested.width, requested.height) ||
            requested.frame_rate.numerator <= 0 || requested.frame_rate.denominator <= 0 ||
            !supported_rate(static_cast<std::uint32_t>(requested.frame_rate.denominator),
                            static_cast<std::uint32_t>(requested.frame_rate.numerator))) {
            fail("unsupported V4L2 capture mode"); cleanup(); return;
        }
        ...
        if (xioctl(video_fd, VIDIOC_S_FMT, &format) != 0) {
            fail(errno_text("VIDIOC_S_FMT"), errno); cleanup(); return;
        }
        if (format.fmt.pix.width != requested.width || format.fmt.pix.height != requested.height ||
            format.fmt.pix.pixelformat != *fourcc || format.fmt.pix.field == V4L2_FIELD_INTERLACED ||
            format.fmt.pix.field == V4L2_FIELD_INTERLACED_TB || format.fmt.pix.field == V4L2_FIELD_INTERLACED_BT) {
            fail("V4L2 device did not accept the requested mode"); cleanup(); return;
        }
        ...
        if (xioctl(video_fd, VIDIOC_S_PARM, &parameters) != 0) {
            fail(errno_text("VIDIOC_S_PARM"), errno); cleanup(); return;
        }
        ...
        if (parameters.parm.capture.timeperframe.numerator != 0 &&
            ...
            if (!supported_rate(numerator, denominator) ||
                numerator > static_cast<std::uint32_t>(std::numeric_limits<std::int32_t>::max()) ||
                denominator > static_cast<std::uint32_t>(std::numeric_limits<std::int32_t>::max())) {
                fail("V4L2 device returned an unsupported frame rate"); cleanup(); return;
            }
            actual.frame_rate = {static_cast<std::int32_t>(denominator),
                                 static_cast<std::int32_t>(numerator)};
        }
        if (actual.frame_rate != requested.frame_rate) {
            fail("V4L2 device did not accept the requested frame rate"); cleanup(); return;
        }
        ...
        if (xioctl(video_fd, VIDIOC_REQBUFS, &request) != 0 || request.count < 1) {
            fail(errno_text("VIDIOC_REQBUFS"), errno); cleanup(); return;
        }
        ...
        for (std::uint32_t i = 0; i < request.count; ++i) {
            ...
            if (xioctl(video_fd, VIDIOC_QUERYBUF, &buffer) != 0) {
                fail(errno_text("VIDIOC_QUERYBUF"), errno); cleanup(); return;
            }
            mappings[i].length = buffer.length;
            mappings[i].address = ::mmap(nullptr, buffer.length, PROT_READ | PROT_WRITE, MAP_SHARED,
                                         video_fd, buffer.m.offset);
            if (mappings[i].address == MAP_FAILED) {
                fail(errno_text("mmap"), errno); cleanup(); return;
            }
            if (xioctl(video_fd, VIDIOC_QBUF, &buffer) != 0) {
                fail(errno_text("VIDIOC_QBUF"), errno); cleanup(); return;
            }
        }
        v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if (xioctl(video_fd, VIDIOC_STREAMON, &type) != 0) {
            fail(errno_text("VIDIOC_STREAMON"), errno); cleanup(); return;
        }
        streaming = true;
        {
            std::lock_guard lock(mutex_);
        ...
        while (!stop_requested_.load(std::memory_order_acquire)) {
            ...
            if (poll_result < 0) {
                ...
                fail(errno_text("poll"), errno); break;
            }
            if (fds[1].revents & POLLIN) break;
            if (fds[0].revents & (POLLERR | POLLHUP | POLLNVAL)) {
                fail("V4L2 device disconnected"); break;
            }
            ...
            if (!newest) continue;
            auto buffer = *newest;
            auto sample = copy_sample(actual, format.fmt.pix, buffer, mappings[buffer.index]);
            if (xioctl(video_fd, VIDIOC_QBUF, &buffer) != 0) {
                fail(errno_text("VIDIOC_QBUF"), errno); break;
            }
            if (sample) publish(std::move(*sample));
        }
        {
            std::lock_guard lock(mutex_);
            if (snapshot_.state == CaptureState::streaming) snapshot_.state = CaptureState::stopping;
        }
        cleanup();
    }

    std::optional<CaptureSample> copy_sample(const CaptureMode& mode, const v4l2_pix_format& format,
                                              const v4l2_buffer& buffer, const Mapping& mapping) {
        if (buffer.bytesused == 0 || buffer.bytesused > mapping.length) return std::nullopt;
        ...
        if (mode.device_format == PixelFormat::mjpeg) {
            ...
            sample = CaptureSample::make_mjpeg(generation, sequence, arrival, mode.width, mode.height,
            ...
            if (mode.device_format == PixelFormat::yuy2 || mode.device_format == PixelFormat::uyvy) {
                if (stride < static_cast<std::size_t>(mode.width) * 2U) return std::nullopt;
                planes.push_back({0, static_cast<std::ptrdiff_t>(stride),
                                  static_cast<std::size_t>(mode.width) * 2U, mode.height});
            } else if (mode.device_format == PixelFormat::nv12) {
                if (stride < mode.width) return std::nullopt;
                if (mode.height > std::numeric_limits<std::size_t>::max() / stride) return std::nullopt;
                const std::size_t y_size = stride * mode.height;
                if (y_size > buffer.bytesused) return std::nullopt;
                planes.push_back({0, static_cast<std::ptrdiff_t>(stride), mode.width, mode.height});
                planes.push_back({y_size, static_cast<std::ptrdiff_t>(stride), mode.width,
                                  (static_cast<std::size_t>(mode.height) + 1U) / 2U});
            } else {
                return std::nullopt;
            }
            if (buffer.bytesused > kMaxRawSampleBytes) return std::nullopt;
            sample = CaptureSample::make_raw(generation, sequence, arrival, mode.width, mode.height,
                                              mode.device_format, planes, {bytes, buffer.bytesused});
        }
        if (sample) {
            sample->device_timestamp = static_cast<std::int64_t>(buffer.timestamp.tv_sec) * 1'000'000LL +
