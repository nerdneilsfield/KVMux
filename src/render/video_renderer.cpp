...
unsigned int compile(unsigned int type, const char* source,
  ...
  if (!ok) {
    ...
    glGetShaderInfoLog(shader, static_cast<int>(log.size()), nullptr,
                       log.data());
    error = log.data();
...
ColorRange effective_range(const VideoFrame& frame, ColorOverride value) {
  if (value == ColorOverride::bt601_full || value == ColorOverride::bt709_full)
    return ColorRange::full;
  if (value == ColorOverride::bt601_limited ||
      value == ColorOverride::bt709_limited)
    return ColorRange::limited;
  auto range = frame.color_range;
  if (range == ColorRange::unknown && frame.frame)
    range = frame_color_range(*frame.frame);
...
ColorMatrix effective_matrix(const VideoFrame& frame, ColorOverride value) {
  if (value == ColorOverride::bt601_full ||
      value == ColorOverride::bt601_limited)
    return ColorMatrix::bt601;
  if (value == ColorOverride::bt709_full ||
      value == ColorOverride::bt709_limited)
...
bool VideoRenderer::compile_program() {
  ...
  if (!ok) {
    std::array<char, 1024> log{};
    glGetProgramInfoLog(program_, static_cast<int>(log.size()), nullptr,
                        log.data());
...
bool VideoRenderer::upload_plane(int index, int width, int height,
                                 unsigned int format, const std::uint8_t* data,
                                 int stride, int row_bytes) {
  if (!data || width <= 0 || height <= 0 || std::abs(stride) < row_bytes)
    return false;
  const std::uint8_t* upload = data;
  int upload_stride = stride;
  if (stride < 0) {
    auto& stage = staging_[static_cast<std::size_t>(index)];
    stage.resize(static_cast<std::size_t>(row_bytes) *
  ...
  const int channels =
      format == GL_RGBA || format == GL_BGRA ? 4 : (format == GL_RG ? 2 : 1);
  glPixelStorei(GL_UNPACK_ROW_LENGTH, upload_stride / channels);
  const unsigned int internal =
      channels == 4 ? GL_RGBA8 : (channels == 2 ? GL_RG8 : GL_R8);
...
bool VideoRenderer::upload(const VideoFrame& input,
                           ColorOverride color_override) {
  snapshot_.error.clear();
  snapshot_.cpu_fallback = false;
  if (!input.frame || !initialize() ||
      !ensure_output(input.frame->width, input.frame->height))
    return false;
  ...
  switch (static_cast<AVPixelFormat>(frame->format)) {
    case AV_PIX_FMT_YUYV422:
      kind = 0;
      snapshot_.pixel_path = "GPU YUY2";
      break;
    case AV_PIX_FMT_UYVY422:
      kind = 1;
      snapshot_.pixel_path = "GPU UYVY";
      break;
    case AV_PIX_FMT_NV12:
      kind = 2;
      snapshot_.pixel_path = "GPU NV12";
      break;
    case AV_PIX_FMT_YUVJ420P:  // Same planes; full range comes from frame
                               // metadata.
    case AV_PIX_FMT_YUV420P:
      kind = 3;
      snapshot_.pixel_path = "GPU YUV420P";
      break;
    case AV_PIX_FMT_YUVJ422P:  // Same planes; full range comes from frame
                               // metadata.
    case AV_PIX_FMT_YUV422P:
      kind = 4;
      snapshot_.pixel_path = "GPU YUV422P";
      break;
    case AV_PIX_FMT_YUVJ444P:  // Same planes; full range comes from frame
                               // metadata.
    case AV_PIX_FMT_YUV444P:
      kind = 5;
      snapshot_.pixel_path = "GPU YUV444P";
      break;
    case AV_PIX_FMT_BGRA:
      kind = 6;
      snapshot_.pixel_path = "GPU BGRA";
      break;
    case AV_PIX_FMT_RGBA:
      kind = 7;
      snapshot_.pixel_path = "GPU RGBA";
      break;
    default:
      break;
  }
  if (kind < 0) {
    ...
    fallback_frame_ =
        AvFramePtr(converted, [](AVFrame* value) { av_frame_free(&value); });
    if (!sws_ || !converted) {
      snapshot_.error = "Could not create CPU pixel fallback";
      return false;
    }
    converted->format = AV_PIX_FMT_RGBA;
    ...
    if (av_frame_get_buffer(converted, 32) < 0 ||
      ...
      snapshot_.error = "CPU pixel fallback failed";
      return false;
    }
    frame = converted;
    kind = 7;
    snapshot_.pixel_path = "CPU swscale to RGBA";
    snapshot_.cpu_fallback = true;
  }

  bool uploaded{};
  if (kind <= 1) {
    if ((frame->width & 1) != 0) {
      snapshot_.error = "Packed 4:2:2 width must be even";
      return false;
    }
    uploaded =
        upload_plane(0, frame->width / 2, frame->height, GL_RGBA,
    ...
    uploaded = upload_plane(0, frame->width, frame->height, GL_RED,
               ...
               upload_plane(1, (frame->width + 1) / 2, (frame->height + 1) / 2,
                            GL_RG, frame->data[1], frame->linesize[1],
                            ((frame->width + 1) / 2) * 2);
  } else if (kind >= 3 && kind <= 5) {
    const int cw = kind == 5 ? frame->width : (frame->width + 1) / 2;
    const int ch = kind == 3 ? (frame->height + 1) / 2 : frame->height;
    uploaded =
        upload_plane(0, frame->width, frame->height, GL_RED, frame->data[0],
                     frame->linesize[0], frame->width) &&
        upload_plane(1, cw, ch, GL_RED, frame->data[1], frame->linesize[1],
                     cw) &&
        upload_plane(2, cw, ch, GL_RED, frame->data[2], frame->linesize[2], cw);
  } else {
    uploaded = upload_plane(0, frame->width, frame->height,
                            kind == 6 ? GL_BGRA : GL_RGBA, frame->data[0],
                            frame->linesize[0], frame->width * 4);
  }
  if (!uploaded) {
    snapshot_.error = "Invalid video plane layout";
    return false;
  }

  const auto matrix = effective_matrix(input, color_override);
  const auto range = effective_range(input, color_override);
  snapshot_.color_choice =
      std::string(matrix == ColorMatrix::bt709 ? "BT.709 " : "BT.601 ") +
      (range == ColorRange::full ? "full" : "limited") +
      (color_override == ColorOverride::automatic
           ? ((input.color_matrix == ColorMatrix::unknown ||
               input.color_range == ColorRange::unknown)
                  ? " (inferred)"
                  : " (metadata)")
           : " (override)");
  const std::array<float, 9> m =
      matrix == ColorMatrix::bt709
          ? std::array<float, 9>{1,       1,       1,       0, -.1873F,
                                 1.8556F, 1.5748F, -.4681F, 0}
          : std::array<float, 9>{1,      1,      1,         0, -.344136F,
                                 1.772F, 1.402F, -.714136F, 0};
  const std::array<float, 3> offset =
      range == ColorRange::full
          ? std::array<float, 3>{0, -.5F, -.5F}
          : std::array<float, 3>{-16.F / 255.F, -128.F / 255.F, -128.F / 255.F};
  const std::array<float, 3> scale =
      range == ColorRange::full
          ? std::array<float, 3>{1, 1, 1}
          : std::array<float, 3>{255.F / 219.F, 255.F / 224.F, 255.F / 224.F};
  ...
  glUniformMatrix3fv(glGetUniformLocation(program_, "yuv_matrix"), 1, GL_FALSE,
                     m.data());
  ...
  if (glGetError() != GL_NO_ERROR) {
    snapshot_.error = "OpenGL video conversion failed";
    return false;
  }
  return true;
}
...
void VideoRenderer::destroy() noexcept {
  ...
  sws_ = nullptr;
  fallback_frame_.reset();
  if (output_texture_) glDeleteTextures(1, &output_texture_);
  if (planes_[0])
    glDeleteTextures(static_cast<int>(planes_.size()), planes_.data());
  ...
  planes_.fill(0);
  width_ = height_ = 0;
  // New GL objects have no storage, even when the next stream has the same
  // size.
  plane_widths_.fill(0);
  plane_heights_.fill(0);
  plane_formats_.fill(0);
}
