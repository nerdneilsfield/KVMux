...
class ControlSink {
 public:
  ...
  virtual void video_presented(std::uint64_t generation,
                               std::uint64_t sequence) noexcept {
    (void)generation;
    (void)sequence;
  }
  [[nodiscard]] virtual SubmitResult submit(ControlEvent event) = 0;
  [[nodiscard]] virtual SubmitResult synchronize(InputSync) {
    return SubmitResult::not_ready;
  }
  // Starts one fully mapped HID-only job. Unsupported sinks return not_ready.
  [[nodiscard]] virtual SubmitResult start_ascii_paste(AsciiPasteJob) {
    return SubmitResult::not_ready;
  }
  ...
  [[nodiscard]] virtual SubmitResult prepare_ascii_paste(AsciiPasteRequest) {
    return SubmitResult::not_ready;
  }
  // Remote sinks receive normalized transaction bytes; local sinks receive HID
  // gestures.
  [[nodiscard]] virtual SubmitResult start_ascii_paste_text(
      std::vector<std::uint8_t>) {
    return SubmitResult::not_ready;
  }
  virtual void cancel_ascii_paste() noexcept {}
  [[nodiscard]] virtual AsciiPasteSnapshot ascii_paste_snapshot() const {
    return {};
  }
