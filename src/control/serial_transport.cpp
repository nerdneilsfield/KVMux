...
bool serial_result(const char* operation, const int result) {
    if (result >= SP_OK) { return true; }
    // Only SP_ERR_FAIL carries an OS error; other results must not read stale errno.
    if (result == SP_ERR_FAIL) {
        ...
        if (message) { sp_free_error_message(message); }
    } else {
        const char* reason = result == SP_ERR_ARG ? "invalid argument" :
            result == SP_ERR_MEM ? "allocation failed" :
            result == SP_ERR_SUPP ? "operation not supported" : "unknown result";
        spdlog::debug("Serial {} failed: result={} reason={}", operation, result, reason);
    }
    return false;
}
...
public:
    bool open(const std::string& name, const int baud_rate) {
        close();
        spdlog::debug("Serial open: path={} baud={} format=8N1 flow_control=none", name, baud_rate);
        sp_port* candidate = nullptr;
        if (!serial_result("get_port_by_name", sp_get_port_by_name(name.c_str(), &candidate))) { return false; }
        port_.reset(candidate);
        if (!serial_result("open", sp_open(port_.get(), SP_MODE_READ_WRITE)) ||
            !serial_result("set_baudrate", sp_set_baudrate(port_.get(), baud_rate)) ||
            !serial_result("set_bits", sp_set_bits(port_.get(), 8)) ||
            !serial_result("set_parity", sp_set_parity(port_.get(), SP_PARITY_NONE)) ||
            !serial_result("set_stopbits", sp_set_stopbits(port_.get(), 1)) ||
            !serial_result("set_flowcontrol", sp_set_flowcontrol(port_.get(), SP_FLOWCONTROL_NONE))) {
            close();
            return false;
        }
    ...
    void close() noexcept {
        if (port_) { (void)serial_result("close", sp_close(port_.get())); }
        port_.reset();
    }

    std::ptrdiff_t write(const std::span<const std::uint8_t> bytes) {
        if (!port_) { return -1; }
        const auto result = sp_nonblocking_write(port_.get(), bytes.data(), bytes.size());
    ...
    std::ptrdiff_t read(const std::span<std::uint8_t> bytes) {
        if (!port_) { return -1; }
        const auto result = sp_nonblocking_read(port_.get(), bytes.data(), bytes.size());
...
std::vector<SerialPortInfo> enumerate_serial_ports() {
    sp_port** raw = nullptr;
    if (!serial_result("list_ports", sp_list_ports(&raw))) { return {}; }
