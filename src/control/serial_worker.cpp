...
std::uint16_t absolute_coordinate(double value) {
  return static_cast<std::uint16_t>(
      std::min(4095.0, std::floor(4096.0 * std::clamp(value, 0.0, 1.0))));
}

bool keyboard_and_mouse_mode(std::uint8_t mode) {
  return mode == 0x00U || mode == 0x80U;
}
...
struct Ch9329ControlSink::Impl {
  ...
  explicit Impl(SerialIo value) : io(std::move(value)) {
    // run() uses members declared after worker; start only after
    // initialization.
  ...
  void cancel_paste_locked(AsciiPasteState state) {
    if (pending_paste) {
      paste = {state, pending_paste->job.gestures.size(), 0,
               pending_paste->job_id};
      pending_paste.reset();
    }
    if (paste.state == AsciiPasteState::active) {
      paste.state = state;
    }
    paste_job.reset();
  ...
  void fail(std::string message, bool reconnect) {
    if (transaction) {
      const auto elapsed =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              Clock::now() - transaction->started)
              .count();
      spdlog::debug(
          "CH9329 transaction failure: kind={} source_sequence={} epoch={} "
          "elapsed_ms={} command=0x{:02x} written={} total={} reason={} "
          "reconnect={}",
          transaction->kind, transaction->source_sequence, transaction->epoch,
          elapsed, transaction->frame.command, transaction->written,
          transaction->bytes.size(), message, reconnect);
    } else {
      spdlog::debug("CH9329 failure: reason={} reconnect={}", message,
                    reconnect);
    }
    io.close();
    open = false;
    ...
    clear_step = ClearStep::none;
    {
      ...
      status.state = reconnect ? ControlConnectionState::reconnecting
                               : ControlConnectionState::fault;
  ...
  void begin(ch9329::Frame frame, Purpose purpose, Clock::time_point now,
    ...
    const char* kind =
        purpose == Purpose::info     ? "info"
        : purpose == Purpose::config ? "config"
        : (purpose == Purpose::sync_keyboard || purpose == Purpose::sync_mouse)
            ? "sync"
        : clear_step != ClearStep::none ? "clear"
                                        : "keyboard";
    Transaction next{
        std::move(frame), purpose, {},   0,        {},         false,
        source_sequence,  epoch,   kind, ordinary, ascii_paste};
    ...
    if (purpose == Purpose::info || purpose == Purpose::config) {
      ...
      spdlog::debug(
          "CH9329 keyboard transaction begin: source_sequence={} epoch={}",
  ...
  void begin_info(Clock::time_point now) {
    begin({address, static_cast<std::uint8_t>(ch9329::Command::get_info), {}},
          Purpose::info, now);
    last_info = now;
  }

  void begin_config(Clock::time_point now) {
    begin({address, static_cast<std::uint8_t>(ch9329::Command::get_config), {}},
          Purpose::config, now);
  }
  ...
  void finish_clear_step(Clock::time_point now) {
    if (clear_step == ClearStep::keyboard) {
      if (clear_both_mouse_modes || mouse_mode == MouseMode::absolute) {
        ...
        begin(ch9329::absolute_mouse_report(address, 0, absolute_x, absolute_y,
                                            0),
              Purpose::absolute, now);
      } else {
        clear_step = ClearStep::relative;
        begin(ch9329::relative_mouse_report(address, 0, 0, 0, 0),
              Purpose::relative, now);
      }
  ...
  bool accept_ack(const ch9329::Frame& reply, Clock::time_point now) {
    if (!transaction || reply.address != address) {
      return false;
    }
    const auto request = transaction->frame.command;
    const bool diagnostic = transaction->purpose == Purpose::info ||
                            transaction->purpose == Purpose::config;
    const bool error_ack =
        reply.command == static_cast<std::uint8_t>(request | 0xc0U);
    if (diagnostic || error_ack) {
      spdlog::debug(
          "CH9329 ACK: request=0x{:02x} reply=0x{:02x} type={} "
          "response_length={}",
          request, reply.command,
          error_ack ? "error"
          : reply.command == static_cast<std::uint8_t>(request | 0x80U)
              ? "normal"
              : "unexpected",
          reply.data.size());
    }
    if (error_ack) {
      spdlog::debug(
          "CH9329 error ACK: command=0x{:02x} status={}", request,
    ...
    if (reply.command == static_cast<std::uint8_t>(request | 0xc0U)) {
      fail("CH9329 returned error status " +
               std::to_string(reply.data.empty() ? -1 : reply.data.front()),
           true);
      return true;
    }
    if (reply.command != static_cast<std::uint8_t>(request | 0x80U)) {
      return false;
    }
    ...
    if (purpose == Purpose::info) {
      ...
      spdlog::debug(
          "CH9329 HID ACK failed: command=0x{:02x} response_length={} "
          "status={}",
          request, reply.data.size(),
          reply.data.empty() ? -1 : static_cast<int>(reply.data.front()));
    ...
    if (was_ascii_paste) {
      std::lock_guard lock(mutex);
      if (paste.state == AsciiPasteState::active && !was_stalled &&
          !release_requested) {
        ++paste_edge;
        if (paste_job &&
            paste_edge == paste_job->gestures[paste_gesture].size()) {
          paste_edge = 0;
          ++paste_gesture;
    ...
    if (purpose == Purpose::info) {
      ...
      spdlog::debug(
          "CH9329 GET_INFO: response_length={} chip_version=0x{:02x} "
      ...
      if (!last_keyboard_leds || *last_keyboard_leds != reply.data[2]) {
        spdlog::debug(
            "CH9329 target keyboard LEDs: bits=0x{:02x} caps_lock={} "
            "num_lock={} scroll_lock={}",
            reply.data[2], (reply.data[2] & 2U) != 0, (reply.data[2] & 1U) != 0,
            (reply.data[2] & 4U) != 0);
      ...
      update_snapshot([&](auto& value) {
        ...
        value.state = !usb_ready
                          ? ControlConnectionState::monitoring
                          : (configuration_validated && !handshake_needs_clear
                                 ? ControlConnectionState::ready
                                 : ControlConnectionState::clearing);
        if (usb_ready && configuration_validated && !handshake_needs_clear)
          queue.set_ready(true);
      });
      if (!usb_ready) {
        std::lock_guard lock(mutex);
      ...
      const bool valid = keyboard_and_mouse_mode(reply.data[0]) &&
                         protocol_mode(reply.data[1]);
      spdlog::debug(
          "CH9329 GET_CONFIG: response_length={} chip_mode=0x{:02x} "
          "protocol_mode=0x{:02x} valid={}",
          reply.data.size(), reply.data[0], reply.data[1], valid);
      if (!valid) {
        fail("CH9329 must expose keyboard+mouse in binary protocol mode",
             false);
      } else {
        ...
        if (canceled) {
          ...
          begin(state.mode == MouseMode::absolute
                    ? ch9329::absolute_mouse_report(address, state.buttons,
                                                    state.absolute_x,
                                                    state.absolute_y, 0)
                    : ch9329::relative_mouse_report(address, state.buttons, 0,
                                                    0, 0),
  ...
  std::optional<std::pair<ch9329::Frame, Purpose>> event_frame(
      ControlEvent event) {
    if (const auto* edge = std::get_if<KeyEdge>(&event.payload)) {
      if (edge->pressed) {
        (void)keyboard.press(edge->usage);
      } else {
        keyboard.release(edge->usage);
      }
      return std::pair{ch9329::keyboard_report(address, keyboard.modifiers(),
                                               keyboard.keys()),
                       Purpose::keyboard};
    }
    if (const auto* motion = std::get_if<AbsoluteMotion>(&event.payload)) {
      ...
      return std::pair{ch9329::absolute_mouse_report(address, buttons,
                                                     absolute_x, absolute_y, 0),
                       Purpose::absolute};
    }
    if (const auto* edge = std::get_if<ButtonEdge>(&event.payload)) {
      const std::uint8_t bit =
          edge->button < 3U ? static_cast<std::uint8_t>(1U << edge->button)
                            : 0U;
      if (edge->pressed) {
        buttons = static_cast<std::uint8_t>(buttons | bit);
      } else {
        buttons = static_cast<std::uint8_t>(buttons & ~bit);
      }
      ...
      if (mouse_mode == MouseMode::absolute) {
        return std::pair{ch9329::absolute_mouse_report(
                             address, buttons, absolute_x, absolute_y, 0),
                         Purpose::absolute};
      }
      return std::pair{ch9329::relative_mouse_report(address, buttons, 0, 0, 0),
                       Purpose::relative};
    }
    ...
    const auto dx =
        std::clamp(static_cast<int>(std::trunc(relative_x)), -127, 127);
    const auto dy =
        std::clamp(static_cast<int>(std::trunc(relative_y)), -127, 127);
    ...
    if (mouse_mode == MouseMode::absolute && dx == 0 && dy == 0) {
      return std::pair{ch9329::absolute_mouse_report(
                           address, buttons, absolute_x, absolute_y,
                           static_cast<std::int8_t>(dw)),
                       Purpose::absolute};
    }
    return std::pair{
        ch9329::relative_mouse_report(
            address, buttons, static_cast<std::int8_t>(dx),
            static_cast<std::int8_t>(dy), static_cast<std::int8_t>(dw)),
        Purpose::relative};
  }
  ...
  void service(Clock::time_point now) {
    ...
    if (!open) {
      ...
      spdlog::debug("CH9329 opening: path={} baud={} address=0x{:02x}",
                    selected_port, selected_baud, address);
      if (!io.open(selected_port, selected_baud)) {
        spdlog::debug("CH9329 open failed: path={} baud={}", selected_port,
                      selected_baud);
    ...
    if (count < 0) {
      fail("Serial read failed; release is unconfirmed", true);
      return;
    }
    if (count > 0) {
      const auto replies =
          parser.feed(std::span(input).first(static_cast<std::size_t>(count)));
      for (const auto& reply : replies) {
        if (accept_ack(reply, now)) {
          break;
        }
      }
    }
    ...
    if (transaction &&
        (transaction->purpose == Purpose::sync_keyboard ||
         transaction->purpose == Purpose::sync_mouse) &&
    ...
    if (transaction) {
      if (transaction->written < transaction->bytes.size()) {
        const auto remaining =
            std::span(transaction->bytes).subspan(transaction->written);
        const auto written = io.write(remaining);
        if (written < 0) {
          fail("Serial write failed; release is unconfirmed", true);
          return;
        }
      ...
      if (elapsed >= kHardTimeout) {
        spdlog::debug(
            "CH9329 transaction timeout: kind={} source_sequence={} epoch={} "
        ...
        spdlog::debug(
            "CH9329 transaction stalled: kind={} source_sequence={} epoch={} "
            "command=0x{:02x} elapsed_ms={} written={} total={}",
            transaction->kind, transaction->source_sequence, transaction->epoch,
            transaction->frame.command,
            std::chrono::duration_cast<std::chrono::milliseconds>(elapsed)
...

    {
      ...
      if (pending_paste) {
        ...
        const bool owner_matches =
            request.owner_epoch == status.epoch && request.owner_intent != 0 &&
            applied.known && applied.epoch == request.owner_epoch &&
            applied.intent_generation == request.owner_intent;
        const bool ready_for_paste =
            status.state == ControlConnectionState::ready &&
            status.target_usb_ready && status.release_confirmed &&
            control_active && now - heartbeat <= kHeartbeatTimeout &&
            !input_sync && !sync_inflight && !ordinary_inflight &&
            queue.size() == 0 && !transaction;
        if (!owner_matches) {
          paste = {AsciiPasteState::canceled, request.job.gestures.size(), 0,
                   request.job_id};
          pending_paste.reset();
        } else if (ready_for_paste) {
          paste = {AsciiPasteState::active, request.job.gestures.size(), 0,
                   request.job_id};
      ...
      if (input_sync) {
        if (!sync_started) {
          ...
          begin(ch9329::keyboard_report(address, input_sync->state.modifiers,
                                        input_sync->state.keys),
                Purpose::sync_keyboard, now);
        }
        return;
      }
      if (paste.state == AsciiPasteState::active && paste_job) {
        ...
        begin(ch9329::keyboard_report(address, keyboard.modifiers(),
                                      keyboard.keys()),
      ...
      if (auto event = queue.pop()) {
        ...
        if (auto frame = event_frame(std::move(*event))) {
          begin(std::move(frame->first), frame->second, now, event->sequence,
                event->epoch, true);
        }
        return;
      }
    }
    if (now - last_info >= kInfoInterval) {
      begin_info(now);
    }
  }
...
Ch9329ControlSink::Ch9329ControlSink()
    : Ch9329ControlSink(make_libserialport_io()) {}
Ch9329ControlSink::Ch9329ControlSink(SerialIo io)
    : impl_(std::make_unique<Impl>(std::move(io))) {}
Ch9329ControlSink::~Ch9329ControlSink() = default;

void Ch9329ControlSink::connect(std::string port, int baud_rate,
                                std::uint8_t address) {
...
SubmitResult Ch9329ControlSink::submit(ControlEvent event) {
  std::lock_guard lock(impl_->mutex);
  if (impl_->paste.state == AsciiPasteState::active || impl_->input_sync ||
      impl_->sync_inflight) {
  ...
  if (result == SubmitResult::accepted)
    impl_->status.ordinary_input_pending = true;
  if (result != SubmitResult::accepted) {
    ++impl_->status.rejected_events;
  }
  impl_->status.epoch = impl_->queue.epoch();
...
SubmitResult Ch9329ControlSink::synchronize(InputSync sync) {
  ...
  bool valid =
      sync.intent_generation != 0 && sync.revision != 0 && state.buttons <= 7 &&
      state.absolute_x <= 4095 && state.absolute_y <= 4095 &&
      (state.mode == MouseMode::absolute || state.mode == MouseMode::relative);
  std::array<bool, 256> seen{};
  for (const auto key : state.keys) {
    if (key == 0) continue;
    const bool supported =
        (key >= 0x04U && key <= 0x73U) || (key >= 0x7fU && key <= 0x82U) ||
        (key >= 0x85U && key <= 0x87U) || (key >= 0x89U && key <= 0x8fU);
  ...
  if (!valid || impl_->status.state != ControlConnectionState::ready ||
      ...
      !impl_->control_active ||
      Clock::now() - impl_->heartbeat > kHeartbeatTimeout) {
    ++impl_->status.rejected_events;
    return SubmitResult::not_ready;
  }
  if (impl_->paste.state == AsciiPasteState::active || impl_->input_sync ||
      impl_->sync_inflight || impl_->ordinary_inflight ||
      impl_->queue.size() != 0 || impl_->relative_x != 0 ||
      impl_->relative_y != 0 || impl_->wheel != 0) {
    ++impl_->status.rejected_events;
    return SubmitResult::overloaded;
  }
...
SubmitResult Ch9329ControlSink::start_ascii_paste(AsciiPasteJob job) {
  std::lock_guard lock(impl_->mutex);
  if (job.gestures.empty() ||
      std::ranges::any_of(job.gestures,
                          [](const auto& edges) { return edges.empty(); }) ||
      impl_->status.state != ControlConnectionState::ready ||
      !impl_->status.target_usb_ready || !impl_->status.release_confirmed ||
      !impl_->control_active ||
      Clock::now() - impl_->heartbeat > kHeartbeatTimeout ||
      impl_->input_sync || impl_->sync_inflight || impl_->ordinary_inflight ||
      impl_->queue.size() != 0 || impl_->transaction ||
      impl_->paste.state == AsciiPasteState::active) {
    ++impl_->status.rejected_events;
    return SubmitResult::not_ready;
  }
...
SubmitResult Ch9329ControlSink::prepare_ascii_paste(AsciiPasteRequest request) {
  std::lock_guard lock(impl_->mutex);
  if (!request.job_id || !request.owner_epoch || !request.owner_intent ||
      request.job.gestures.empty() ||
      std::ranges::any_of(request.job.gestures,
                          [](const auto& edges) { return edges.empty(); }) ||
...
void Ch9329ControlSink::cancel_ascii_paste() noexcept {
  ...
  if (impl_->pending_paste) {
    impl_->paste = {AsciiPasteState::canceled,
                    impl_->pending_paste->job.gestures.size(), 0,
                    impl_->pending_paste->job_id};
