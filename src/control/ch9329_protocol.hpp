...
[[nodiscard]] Frame keyboard_report(
    std::uint8_t address, std::uint8_t modifiers,
    std::span<const std::uint8_t, 6> usages);
[[nodiscard]] Frame absolute_mouse_report(
    std::uint8_t address, std::uint8_t buttons, std::uint16_t x,
    std::uint16_t y, std::int8_t wheel);
[[nodiscard]] Frame relative_mouse_report(
    std::uint8_t address, std::uint8_t buttons, std::int8_t dx,
