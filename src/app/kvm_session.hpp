...
// Main-thread coordinator. Its public event and tick functions are intended to be
// called by the UI thread; capture lifecycle work is performed by its worker.
class KvmSession {
public:
    ...
    [[nodiscard]] std::future<std::vector<DeviceInfo>> enumerate_capture_devices();
    [[nodiscard]] std::future<std::vector<CaptureMode>> enumerate_capture_modes(std::string stable_id);
    [[nodiscard]] bool select_capture(const DeviceInfo& device, const CaptureMode& mode);
    ...
    void shutdown(std::chrono::milliseconds serial_timeout = std::chrono::milliseconds(500)) noexcept;
