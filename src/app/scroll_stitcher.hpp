...
enum class StitchResult { started, appended, unchanged, bad_match, reverse, stale, size_limit, invalid };
...
public:
    ...
    StitchUpdate add(std::span<const std::uint8_t> packed_rgb, std::uint32_t width,
                     std::uint32_t height, std::uint64_t sequence);
    void reset() noexcept;

    [[nodiscard]] std::uint32_t width() const noexcept { return width_; }
    [[nodiscard]] std::uint32_t height() const noexcept;
    [[nodiscard]] std::span<const std::uint8_t> pixels() const noexcept { return output_; }
