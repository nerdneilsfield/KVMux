...
inline void accept_paste_execute(PasteExecuteRetry& retry,
                                 std::uint64_t challenge) noexcept {
  retry.accepted_challenge = challenge;
}

inline bool should_retry_paste_execute(
    PasteExecuteRetry& retry, std::uint64_t challenge,
    std::chrono::steady_clock::time_point now) noexcept {
  using namespace std::chrono_literals;
  if (challenge <= retry.accepted_challenge ||
      (challenge == retry.attempted_challenge &&
       now - retry.attempted_at < 10ms))
    return false;
