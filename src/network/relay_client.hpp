...
struct PasteUploadSnapshot {
  ...
  [[nodiscard]] bool terminal() const noexcept {
    return state == PasteUploadState::completed ||
           state == PasteUploadState::canceled ||
           state == PasteUploadState::rejected ||
           state == PasteUploadState::expired;
  }
};
...
struct TrafficSnapshot {
  std::uint64_t video_received_bytes{}, control_received_bytes{},
      control_sent_bytes{};
};
class RelayClient {
 public:
  ...
  // Takes already normalized US-ASCII transaction bytes. It never logs or
  // retains them after completion/cancel.
...
class NetworkCaptureSource final : public CaptureSource {
 public:
  explicit NetworkCaptureSource(std::shared_ptr<RelayClient> client)
  ...
  std::optional<CaptureSample> take_latest_sample() override {
    return client_->take_sample();
  }
  CaptureSnapshot snapshot() const override {
    return client_->capture_snapshot();
  }
...
class NetworkControlSink final : public ControlSink {
 public:
  explicit NetworkControlSink(std::shared_ptr<RelayClient> client)
      : client_(std::move(client)) {}
  ...
  void set_control_active(bool value) noexcept override {
    client_->active(value);
  }
  void update_ui_heartbeat() noexcept override { client_->gui_progress(); }
  void video_presented(std::uint64_t generation,
                       std::uint64_t sequence) noexcept override {
    client_->video_presented(generation, sequence);
  }
  kvmux::SubmitResult synchronize(InputSync value) override {
    return client_->synchronize(std::move(value));
  }
  kvmux::SubmitResult submit(ControlEvent event) override {
    return client_->submit(std::move(event));
  }
  void release_all() noexcept override { client_->release(); }
  kvmux::SubmitResult start_ascii_paste_text(
      std::vector<std::uint8_t> normalized) override {
    return client_->start_ascii_paste_text(std::move(normalized));
  }
  void cancel_ascii_paste() noexcept override {
    client_->cancel_ascii_paste_text();
  }
  AsciiPasteSnapshot ascii_paste_snapshot() const override;
  ControlSnapshot snapshot() const override {
    return client_->control_snapshot();
  }
