...
bool better(const CaptureMode& a, const CaptureMode& b) {
    ...
    const auto size_a = std::tuple{static_cast<std::uint64_t>(a.width) * a.height, a.width, a.height};
    const auto size_b = std::tuple{static_cast<std::uint64_t>(b.width) * b.height, b.width, b.height};
    if (size_a != size_b) return size_a > size_b;
    return static_cast<std::int64_t>(a.frame_rate.numerator) * b.frame_rate.denominator >
           static_cast<std::int64_t>(b.frame_rate.numerator) * a.frame_rate.denominator;
}
...
std::string select_device(std::span<const DeviceInfo> devices,
    ...
    std::string error = devices.empty() ? "No capture devices found; specify --device after connecting a device."
...
std::string select_serial(std::span<const SerialPortInfo> ports,
                          const std::optional<std::string>& requested) {
    // Explicit names may not be enumerated by a driver; opening reports their errors.
    if (requested) {
        if (requested->empty()) throw std::runtime_error("Empty --serial port");
        return *requested;
    }
    std::vector<const SerialPortInfo*> candidates;
    for (const auto& port : ports) if (known_adapter(port)) candidates.push_back(&port);
    if (candidates.size() == 1) return candidates.front()->name;
    std::string error = candidates.empty() ? "No known CH340/CH341/CH343 USB serial adapter found; specify --serial."
                                          : "Multiple CH340/CH341/CH343 USB serial adapters; specify --serial. Candidates:";
    for (const auto* port : candidates) error += "\n  " + port->name + " (" + port->description + ")";
    throw std::runtime_error(error);
}
}  // namespace kvmux::relay
