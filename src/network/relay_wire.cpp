...
std::optional<std::vector<std::uint8_t>> encode_control(const Control& c,Direction d) {
    ...
    std::visit([&](const auto& b) {
        ...
        else if constexpr(std::is_same_v<T,PasteExecute>) { type=10; w.ok &= b.transaction_id; w.integer(b.transaction_id,8); }
        ...
        else if constexpr(std::is_same_v<T,PasteStatus>) { type=12; auto state=static_cast<unsigned>(b.state), reason=static_cast<unsigned>(b.reason); w.ok &= b.transaction_id && state>=1 && state<=5 && static_cast<unsigned>(b.outcome)<=4 && reason<=9; w.integer(b.transaction_id,8); w.integer(state,1); w.zeros(3); w.integer(b.next_chunk,4); w.integer(b.accepted_bytes,4); w.integer(b.completed_bytes,4); w.integer(static_cast<unsigned>(b.outcome),1); w.integer(reason,1); w.zeros(2); }
...
std::optional<Control> decode_control(std::span<const std::uint8_t> bytes,Direction d) {
    ...
    if(!h.done()||n!=bytes.size()-4||type<1||type>13||!direction(type,d)) return {};
    ...
    case 10: { PasteExecute b{r.integer(8)}; r.ok &= b.transaction_id; c=b; break; }
    ...
    case 12: { PasteStatus b; b.transaction_id=r.integer(8); auto state=r.u8(); r.zeros(3); b.next_chunk=static_cast<std::uint32_t>(r.integer(4)); b.accepted_bytes=static_cast<std::uint32_t>(r.integer(4)); b.completed_bytes=static_cast<std::uint32_t>(r.integer(4)); auto outcome=r.u8(), reason=r.u8(); r.zeros(2); r.ok &= b.transaction_id && state>=1 && state<=5 && outcome<=4 && reason<=9; if(state>=1&&state<=5) b.state=static_cast<PasteState>(state); if(outcome<=4) b.outcome=static_cast<PasteOutcome>(outcome); if(reason<=9) b.reason=static_cast<PasteStatusReason>(reason); c=b; break; }
    }
