...
struct SessionIds { std::uint64_t session{}, generation{}; std::uint32_t conversation{}; };
...
public:
    ...
    SessionActions sync_submitted(const wire::Sync&, kvmux::SubmitResult, SessionTime);
    ...
    SessionActions edge_submitted(const wire::Edge&, kvmux::SubmitResult, SessionTime);
...
public:
    ...
    SessionActions on_datagram(const udp::Endpoint&, std::span<const std::uint8_t>, SessionTime);
    SessionActions tick(SessionTime);
