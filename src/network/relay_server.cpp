...
struct RelayServer::Impl {
    ...
    try {
      while (!stopping) {
        ...
        std::chrono::microseconds interval;
        {
          ...
          media_wake.wait_for(lock, 2ms, [&] {
            return stopping || generation != media_generation || keyframe;
          });
          if (stopping) break;
        ...
        if (generation != wanted) {
          ...
          if (generation && options.codec != VideoCodec::mjpeg) {
            ...
            settings.fps_numerator =
                static_cast<std::uint32_t>(mode.frame_rate.numerator);
            settings.fps_denominator =
                static_cast<std::uint32_t>(mode.frame_rate.denominator);
            settings.codec = options.codec;
            settings.priority = options.priority;
        ...
        if (codec) {
          ...
          if (result.ok()) {
            ...
            if (bytes.empty())
              throw std::runtime_error("Invalid H.26x access unit");
            waiting_idr = false;
            publish({options.codec, generation, unit.encoded_sequence, unit.idr,
                     std::move(bytes), unit.arrival});
            continue;  // Never overwrite an ordered output slot.
          }
          if (result.status != CodecStatus::again)
        ...
        if (!sample || Clock::now() - sample->arrival >= 250ms) {
        ...
        {
          std::lock_guard lock(media_mutex);
          ++snapshot.source_admissions;
        }
        if (!codec) {
          auto bytes = encode_mjpeg(*sample);
          if (bytes.empty()) throw std::runtime_error("Invalid MJPEG capture");
          publish({VideoCodec::mjpeg, generation, sample->sequence, false,
                   std::move(bytes), sample->arrival});
          continue;
        }
        if (!sample->raw || sample->mjpeg)
          throw std::runtime_error("H.26x capture is not raw");
        ...
        if (input->format != AV_PIX_FMT_NV12 &&
          ...
          if (!converted)
            throw std::runtime_error("H.26x frame allocation failed");
          ...
          if (av_frame_get_buffer(converted.get(), 32) < 0 ||
              av_frame_copy_props(converted.get(), input.get()) < 0)
            throw std::runtime_error("H.26x frame storage allocation failed");
          scaler.reset(sws_getCachedContext(
              scaler.release(), input->width, input->height,
              static_cast<AVPixelFormat>(input->format), input->width,
              input->height, AV_PIX_FMT_NV12, SWS_FAST_BILINEAR, nullptr,
              nullptr, nullptr));
          const int matrix = input->colorspace == AVCOL_SPC_BT709
                                 ? SWS_CS_ITU709
                                 : SWS_CS_ITU601;
          const bool rgb = input->format == AV_PIX_FMT_BGRA ||
                           input->format == AV_PIX_FMT_RGBA;
          const int full =
              rgb || input->color_range == AVCOL_RANGE_JPEG ? 1 : 0;
          converted->color_range = full ? AVCOL_RANGE_JPEG : AVCOL_RANGE_MPEG;
          converted->colorspace =
              matrix == SWS_CS_ITU709 ? AVCOL_SPC_BT709 : AVCOL_SPC_SMPTE170M;
          if (!scaler ||
              sws_setColorspaceDetails(
                  scaler.get(), sws_getCoefficients(matrix), full,
                  sws_getCoefficients(matrix), full, 0, 1 << 16, 1 << 16) < 0 ||
              sws_scale(scaler.get(), input->data, input->linesize, 0,
                        input->height, converted->data,
                        converted->linesize) != input->height)
            throw std::runtime_error("H.26x conversion failed");
          input = std::move(converted);
        }
        EncoderInput value;
        value.frame = std::move(input);
        value.generation = generation;
        value.capture_sequence = sample->sequence;
        value.arrival = sample->arrival;
        value.pts_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                           sample->arrival.time_since_epoch())
                           .count();
        const auto result = codec->submit(value);
        if (!result.ok() && result.status != CodecStatus::again)
          throw std::runtime_error(result.message);
      }
    } catch (const std::exception& error) {
      std::lock_guard lock(media_mutex);
      media_error = error.what();
    }
    reset_codec();
  }
  ...
  void run() {
    ...
    struct Sent {
      ...
      Clock::time_point arrival{}, completed{};
    };
    ...
    bool fatal{};
    auto request_refresh = [&] {
      std::lock_guard lock(media_mutex);
      keyframe = true;
      media_wake.notify_one();
    };
    auto revoke = [&] {
      sink.cancel_ascii_paste();
      sink.set_control_active(false);
      sink.release_all();
      ack.reset();
    };
      ...
      for (const auto& action : batch) {
        switch (action.kind) {
          case SessionAction::Kind::send_raw:
            if (control_socket->send_to(action.peer, action.datagram) ==
                udp::SendStatus::error)
              fatal = true;
            break;
          case SessionAction::Kind::established:
            kcp = std::make_unique<KcpChannel>(session.tuple().conversation);
            pacer = std::make_unique<MediaPacer>(
                options.codec, session.welcome().generation,
                options.transport_bytes_per_second, Clock::now());
            {
              std::lock_guard lock(media_mutex);
              const auto nominal = snapshot.nominal_interval;
              snapshot = {};
              snapshot.nominal_interval = snapshot.admission_interval = nominal;
              snapshot.generation = session.welcome().generation;
              snapshot.transport_bytes_per_second =
                  options.transport_bytes_per_second;
            ...
            {
              std::lock_guard lock(media_mutex);
              media_generation = session.welcome().generation;
              offer.reset();
              credit = true;
              keyframe = true;
            }
            media_wake.notify_one();
            break;
          case SessionAction::Kind::revoke_input:
            revoke();
            break;
          case SessionAction::Kind::state_ack:
            ack = action.ack;
            break;
          case SessionAction::Kind::paste_ready:
            paste_status = action.paste_status;
            if (action.paste_status &&
                (action.paste_status->state == wire::PasteState::finished)) {
              sink.cancel_ascii_paste();
            }
            break;
    ...
    while (!stopping && !fatal) {
      ...
      for (unsigned i = 0; i < 32; ++i) {
        ...
        if (input.status == udp::ReceiveStatus::error) {
          fatal = true;
          break;
        }
        if (input.status == udp::ReceiveStatus::datagram)
          received.push_back(std::move(input.datagram));
      }
      // Raw cancellation overtakes reliable edges already in this bounded
      // batch.
      std::stable_sort(received.begin(), received.end(),
                       [](const auto& a, const auto& b) {
                         auto ea = wire::decode_envelope(a.bytes),
                              eb = wire::decode_envelope(b.bytes);
                         return (ea && ea->kind == wire::EnvelopeKind::cancel) >
                                (eb && eb->kind == wire::EnvelopeKind::cancel);
                       });
      for (const auto& packet : received) {
        ...
        if (envelope->kind == wire::EnvelopeKind::kcp) {
          if (kcp && session.matches(packet.source, envelope->tuple))
            (void)kcp->input(envelope->body);
          continue;
        }
        bool valid_video = false;
        if (envelope->kind == wire::EnvelopeKind::proof &&
            session.matches(packet.source, envelope->tuple)) {
          const auto proof = std::get<wire::Proof>(
              *wire::decode_raw(envelope->kind, envelope->body));
          if (proof.active && proof.video_fresh && sending &&
              sending->sequence == proof.presented_sequence) {
            ...
            if (pending_proof) {
              const auto previous =
                  wire::decode_envelope(pending_proof->packet.bytes);
              const auto value = std::get<wire::Proof>(
                  *wire::decode_raw(previous->kind, previous->body));
              newer = proof.challenge > value.challenge;
            }
            if (newer) pending_proof = PendingProof{packet, now};
            continue;
          }
          valid_video = std::any_of(
              history.begin(), history.end(), [&](const Sent& frame) {
                return frame.sequence == proof.presented_sequence &&
                       now - frame.arrival < 500ms;
              });
        }
        SessionIds ids;
        if (envelope->kind == wire::EnvelopeKind::hello &&
            session.phase() == SessionPhase::idle) {
          ids = {random_id(), random_id(),
                 static_cast<std::uint32_t>(random_id())};
          if (!ids.conversation) ids.conversation = 1;
        }
        actions(session.on_datagram(packet.source, packet.bytes, now, ids,
                                    valid_video));
        if (session.matches(packet.source, envelope->tuple))
          peer = packet.source;
      }
      now = Clock::now();
      if (pending_proof) {
        ...
        const auto proof = std::get<wire::Proof>(
            *wire::decode_raw(envelope->kind, envelope->body));
        const bool complete =
            std::any_of(history.begin(), history.end(), [&](const Sent& frame) {
              return frame.sequence == proof.presented_sequence &&
                     now - frame.arrival < 500ms;
            });
        if (complete) {
          auto pending = std::move(*pending_proof);
          pending_proof.reset();
          // The proof was on time when received. Only a completed frame
          // may authorize it; session leases start at this real instant.
          actions(session.on_datagram(pending.packet.source,
                                      pending.packet.bytes, now, {}, true,
                                      pending.received_at));
      ...
      for (unsigned i = 0; i < 32; ++i) {
        ...
        if (!bytes) break;
        auto message =
            wire::decode_control(*bytes, wire::Direction::client_to_server);
        if (!message) continue;
        now = Clock::now();
        if (!kcp) break;
        if (auto value = std::get_if<wire::Cancel>(&*message))
          actions(session.cancel(*value, now));
        else if (auto value = std::get_if<wire::Sync>(&*message)) {
          if (session.check_sync(*value, now) == InputGate::allowed)
            actions(session.sync_submitted(
                *value,
                sink.synchronize({value->epoch, value->intent, value->revision,
                                  value->state}),
                now));
        } else if (auto value = std::get_if<wire::Edge>(&*message)) {
          auto gate = session.check_edge(*value, now);
          if (gate == InputGate::allowed)
            actions(session.edge_submitted(
                *value,
                sink.submit({value->epoch, value->source_sequence, now,
                             value->payload}),
                now));
          else if (gate == InputGate::recovery_required)
            actions(session.edge_submitted(
                *value, kvmux::SubmitResult::not_ready, now));
        ...
        } else if (auto value = std::get_if<wire::PasteExecute>(&*message)) {
          actions(session.paste_execute(*value, now));
          if (auto bytes = session.pending_paste_bytes()) {
            const auto mapped = map_us_ascii_text(std::string_view(
                reinterpret_cast<const char*>(bytes->data()), bytes->size()));
            if (!mapped) {
              sink.cancel_ascii_paste();
              actions(session.paste_start_failed(now));
            } else {
              AsciiPasteJob job;
              job.gestures.reserve(mapped.gestures.size());
              for (auto& gesture : mapped.gestures)
                job.gestures.push_back(std::move(gesture.edges));
              const auto owner = session.pending_paste_owner();
              const auto job_id = next_paste_job_id++;
              if (owner &&
                  sink.prepare_ascii_paste(
                      {job_id, owner->first, owner->second, std::move(job)}) ==
                      kvmux::SubmitResult::accepted)
          ...
          if (admission &&
              admission->feedback(value->generation, value->stats, now)) {
            std::lock_guard lock(media_mutex);
            ++snapshot.feedback_samples;
          }
        } else if (auto value = std::get_if<wire::RefreshRequest>(&*message)) {
          if (value->generation == session.welcome().generation)
            request_refresh();
        }
        if (!kcp) break;
      }
      ...
      if (now - status_at >= 50ms) {
        ...
        status = wire::Status{value.epoch,
                              value.state,
                              value.target_usb_ready,
                              value.release_confirmed,
                              session.canceled_through(),
                              value.ordinary_input_pending,
                              value.completed_ordinary_sequence};
        status_at = now;
      }
      auto submit = [&](const wire::Control& value) {
        auto bytes =
            wire::encode_control(value, wire::Direction::server_to_client);
      ...
      if (paste_status && now - paste_status_at >= 50ms &&
          submit(*paste_status)) {
        paste_status.reset();
        paste_status_at = now;
      }
      ...
      auto drain_control = [&] {
        for (unsigned i = 0; i < 32 && control_index < control_output.size();
             ++i) {
          auto bytes =
              wire::encode_envelope({wire::EnvelopeKind::kcp, session.tuple(),
                                     control_output[control_index]});
          if (!bytes) {
            fatal = true;
            return false;
          }
      ...
      control_output = kcp->take_datagrams();
      control_index = 0;
      (void)drain_control();
      if (!pacer) continue;
      now = Clock::now();
      ...
      while (!history.empty() &&
             (history.size() > 512 || now - history.front().arrival >= 500ms))
        history.pop_front();
      auto expired = pacer->poll(now);
      if (expired.reason != MediaReason::none) {
        record_reason(expired.reason);
        sending.reset();
        if (expired.needs_idr) request_refresh();
      }
      ...
      if (!pending && pacer->can_start(now)) {
        ...
        {
          std::lock_guard lock(media_mutex);
          frame = std::exchange(offer, std::nullopt);
        }
        if (frame) {
          ...
          if (result.accepted) {
            sending = candidate;
            record_reason(MediaReason::none);
          } else {
            record_reason(result.reason);
            if (result.needs_idr) request_refresh();
          }
      ...
      for (unsigned i = 0; i < 32; ++i) {
        ...
        auto bytes = wire::encode_envelope(
            {wire::EnvelopeKind::media, session.tuple(), pending->body});
        ...
        if (!pacer->active_deadline() && sending) {
          sending->completed = Clock::now();
          history.push_back(*sending);
          sending.reset();
      ...
      {
        std::lock_guard lock(media_mutex);
        if (!pending && pacer->can_start(Clock::now()) && !offer) {
          credit = true;
          media_wake.notify_one();
        }
        if (!media_error.empty()) {
          spdlog::error("Relay media: {}", media_error);
          fatal = true;
        }
      }
    }
...
RelayServer::RelayServer(CaptureSource& c, Ch9329ControlSink& s,
                         EncoderFactory f)
...
bool RelayServer::start(const ServerOptions& options, std::string& error) {
  auto& p = *impl_;
  if (p.network_worker.joinable()) {
    error = "Relay already running";
    return false;
  }
  error.clear();
  if ((options.control_port && options.control_port == options.video_port) ||
      !options.transport_bytes_per_second ||
      options.transport_bytes_per_second > 1'000'000'000) {
    error =
        "UDP ports must differ and transport rate must be 1..1000000000 "
        "bytes/s";
    return false;
  }
  const auto mode = p.capture.snapshot().actual_mode;
  try {
    select_mode(std::span<const CaptureMode>(&mode, 1), 0, options.codec);
  } catch (const std::exception& e) {
    error = e.what();
    return false;
  }
  if (options.codec != VideoCodec::mjpeg &&
      (!options.bitrate || options.bitrate > 100'000'000)) {
    error = "H.26x bitrate must be 1..100000000 bits/s";
    return false;
  }
  p.options = options;
  {
    ...
    p.snapshot.nominal_interval = p.snapshot.admission_interval =
        std::chrono::microseconds(std::max<std::int64_t>(
            1, 1'000'000LL * mode.frame_rate.denominator /
                   mode.frame_rate.numerator));
  }
  p.control_socket =
      udp::Socket::bind(options.bind_address, options.control_port, error);
  if (!p.control_socket) return false;
  p.video_socket =
      udp::Socket::bind(options.bind_address, options.video_port, error);
  if (!p.video_socket) {
    p.control_socket.reset();
    return false;
  }
  p.stopping = false;
  p.media_error.clear();
...
void RelayServer::stop() noexcept {
  auto& p = *impl_;
  p.stopping = true;
  p.media_wake.notify_all();
  if (p.network_worker.joinable()) p.network_worker.join();
  if (p.media_worker.joinable()) p.media_worker.join();
  p.sink.set_control_active(false);
  p.sink.release_all();
  p.control_socket.reset();
  p.video_socket.reset();
}
ServerSnapshot RelayServer::snapshot() const {
  std::lock_guard lock(impl_->media_mutex);
  return impl_->snapshot;
}
std::uint16_t RelayServer::control_port() const {
  return impl_->control_socket ? impl_->control_socket->local_port().value_or(0)
                               : 0;
}
std::uint16_t RelayServer::video_port() const {
  return impl_->video_socket ? impl_->video_socket->local_port().value_or(0)
                             : 0;
}
}  // namespace kvmux::relay
