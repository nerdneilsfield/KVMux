...
bool Endpoint::operator==(const Endpoint& other) const noexcept {
    ...
    return address_->value.sin_addr.s_addr == other.address_->value.sin_addr.s_addr &&
           address_->value.sin_port == other.address_->value.sin_port;
}
std::optional<Endpoint> resolve(std::string_view host, std::uint16_t port, std::string& error) {
    ...
    if (getaddrinfo(name.c_str(), service.c_str(), &hints, &addresses) != 0 || !addresses) {
...
std::optional<Socket> Socket::bind(std::string_view host, std::uint16_t port, std::string& error) {
    ...
    if (!nonblocking(raw) || ::bind(raw,
            reinterpret_cast<const sockaddr*>(&endpoint->address_->value), sizeof(sockaddr_in)) != 0) {
...
std::optional<std::uint16_t> Socket::local_port() const {
    ...
    if (native(handle_) == invalid_socket ||
        getsockname(native(handle_), reinterpret_cast<sockaddr*>(&address), &length) != 0)
        return std::nullopt;
    return ntohs(address.sin_port);
}
SendStatus Socket::send_to(const Endpoint& endpoint, std::span<const std::uint8_t> bytes) const {
    if (!endpoint.address_ || bytes.size() > max_payload) return SendStatus::invalid;
    if (native(handle_) == invalid_socket) return SendStatus::error;
    // Nonblocking: a caller may drop an unsent datagram; never retry a partial one.
    const auto count = ::sendto(native(handle_), reinterpret_cast<const char*>(bytes.data()),
        static_cast<BufferLength>(bytes.size()), 0,
        reinterpret_cast<const sockaddr*>(&endpoint.address_->value), sizeof(sockaddr_in));
    if (count >= 0 && static_cast<std::size_t>(count) == bytes.size()) return SendStatus::sent;
    if (count < 0 && (would_block(last_error()) || interrupted(last_error()))) return SendStatus::would_block;
...
ReceiveResult Socket::receive(std::chrono::milliseconds timeout) const {
    ...
    for (;;) {
        ...
        const auto milliseconds = static_cast<int>(std::clamp<std::int64_t>(remaining.count(), 0, INT_MAX));
#ifdef _WIN32
        fd_set ready;
        FD_ZERO(&ready);
        ...
        if (result < 0) {
            ...
            // One spare byte identifies over-limit datagrams even if recvfrom truncates.
            std::array<std::uint8_t, max_payload + 1> bytes{};
            auto address = std::make_shared<Endpoint::Address>();
            SocketLength length = sizeof(address->value);
            const auto count = ::recvfrom(native(handle_), reinterpret_cast<char*>(bytes.data()),
                static_cast<BufferLength>(bytes.size()), 0, reinterpret_cast<sockaddr*>(&address->value), &length);
#ifdef _WIN32
            if (count < 0 && last_error() == WSAEMSGSIZE) return {ReceiveStatus::oversized, {}};
#endif
            if (count > static_cast<int>(max_payload)) return {ReceiveStatus::oversized, {}};
            if (count >= 0) {
                Endpoint source;
