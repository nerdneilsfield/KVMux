...
// Single owner thread; update every 10 ms, including when the application is
// idle. Transport ACKs do not confirm execution by the serial device.
class KcpChannel {
 public:
  ...
  // Keep reserve_slots entries available for a message that must follow this
  // one.
