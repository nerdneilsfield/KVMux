...
private:
    ...
    friend class Socket;
    friend std::optional<Endpoint> resolve(std::string_view, std::uint16_t, std::string&);
};
[[nodiscard]] std::optional<Endpoint> resolve(std::string_view host, std::uint16_t port,
                                             std::string& error);
struct Datagram {
    Endpoint source;
...
public:
    ...
    [[nodiscard]] static std::optional<Socket> bind(std::string_view host, std::uint16_t port,
                                                   std::string& error);
    [[nodiscard]] std::optional<std::uint16_t> local_port() const;
    [[nodiscard]] SendStatus send_to(const Endpoint&, std::span<const std::uint8_t>) const;
