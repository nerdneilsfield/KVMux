...
struct MediaFrame {
    ...
    MediaTime first_arrival{}; // Sender: local source arrival. Receiver: first packet.
};
enum class MediaReason {
    none, malformed, conflict, invalid_body, age, capacity, gap,
...
// Single owner. Each call returns a bounded batch; caller consumes it synchronously.
// 32 MiB is resident reassembly charge (body, parity, bitmaps, 512 bytes/entry),
// not process RSS. Codec validation temporarily owns at most one extra 16 MiB
// body + 58 bytes/padding. Returned bodies are moved, total <=32 MiB/call;
// at most 32 event records. Do not accumulate batches in another queue.
// Construct a new receiver for a new negotiated generation. Reset markers fence
// decoder output; caller restores first_arrival after decode_* validation/copy.
class MediaReceiver {
public:
...
[[nodiscard]] std::optional<std::vector<std::uint8_t>> media_packet(const MediaFrame&, std::size_t ordinal);
...
struct MediaPacerStats {
    std::uint64_t sent_bytes{}, sender_deadlines{}, rejected_frames{}, skipped_access_units{};
    bool waiting_idr{};
};
// Rate counts outer 32-byte envelope + body, NOT UDP/IP headers. Latest raw source
// stays in the caller's existing mailbox. Caller services control before pulling
// at most two packets and checking control again (2400-byte burst cap).
class MediaPacer {
public:
    MediaPacer(VideoCodec, std::uint64_t generation, std::uint64_t bytes_per_second, MediaTime);
    ~MediaPacer();
