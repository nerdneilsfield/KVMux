...
// Inter-frame payloads contain one complete Annex B access unit, not individual NALs.
...
[[nodiscard]] std::vector<std::uint8_t> encode_annex_b(const EncodedAccessUnit&);
[[nodiscard]] std::optional<EncodedAccessUnit> decode_annex_b(std::span<const std::uint8_t>, VideoCodec);
