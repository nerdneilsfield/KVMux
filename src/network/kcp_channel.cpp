...
KcpChannel::KcpChannel(std::uint32_t conversation)
    : kcp_(ikcp_create(conversation, this)) {
  if (!kcp_) {
    failed_ = true;
    return;
  }
...
SubmitResult KcpChannel::submit(std::span<const std::uint8_t> message,
  ...
  if (failed_ || message.empty() || message.size() > max_message ||
      reserve_slots >= window)
    return SubmitResult::invalid;
  if (ikcp_waitsnd(kcp_) >= static_cast<int>(window - reserve_slots))
    return SubmitResult::full;
  if (ikcp_send(kcp_, reinterpret_cast<const char*>(message.data()),
...
bool KcpChannel::input(std::span<const std::uint8_t> packet) {
  ...
  for (std::size_t offset = 0; offset < packet.size();) {
    if (packet.size() - offset < 24 || read32(packet, offset) != kcp_->conv)
      return false;
    const auto command = packet[offset + 4];
    const auto fragment = packet[offset + 5];
    const auto peer_window =
        unsigned{packet[offset + 6]} | (unsigned{packet[offset + 7]} << 8);
    const auto length = read32(packet, offset + 20);
    // Our 1024-byte messages fit one segment. ACK/probes have no payload.
    if (fragment != 0 || peer_window > window ||
        length > packet.size() - offset - 24)
      return false;
    if (command == 81) {
  ...
  // Upstream ACK storage otherwise grows without bound on duplicate PUSH
  // floods. Dropped input is retried by the peer after the next update flushes
  // our ACKs.
  if (kcp_->ackcount + pushes > max_pending_acks) return false;
  return ikcp_input(kcp_, reinterpret_cast<const char*>(packet.data()),
                    static_cast<long>(packet.size())) == 0;
}
...
int KcpChannel::output(const char* data, int size, IKCPCB*, void* user) {
  ...
  if (size <= 0 || static_cast<std::size_t>(size) > mtu ||
      self.output_.size() >= max_output) {
    self.failed_ = true;
    return -1;
  }
...
std::optional<std::vector<std::uint8_t>> KcpChannel::receive() {
  ...
  if (size == 0 || size > static_cast<int>(max_message)) {
    failed_ = true;
    return std::nullopt;
  }
