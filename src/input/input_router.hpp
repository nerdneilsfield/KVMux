...
struct InputPointerMotion {
  double x{};
  double y{};
};
struct InputRelativeMotion {
  double dx{};
  double dy{};
};
struct InputButton {
...
struct InputWheel {
  double vertical_steps{};
  double x{};
  double y{};
};
using InputPayload = std::variant<InputKey, InputPointerMotion,
                                  InputRelativeMotion, InputButton, InputWheel>;
struct InputEvent {
...
[[nodiscard]] Rect fit_video_rect(Rect area, int video_width,
                                  int video_height) noexcept;
// Centered target desktop inside the rendered capture; does not alter
// rendering.
[[nodiscard]] Rect target_input_rect(Rect display,
                                     TargetAspect aspect) noexcept;
...
struct InputPointerSnapshot {
  std::optional<Rect> video_rect;  // Event-time active desktop rectangle in
                                   // logical window units.
  std::optional<std::pair<double, double>>
      video_local;  // Logical window units.
  std::optional<std::pair<std::uint16_t, std::uint16_t>>
      submitted_absolute;                                 // 0..4095.
...
class InputRouter {
 public:
  ...
  [[nodiscard]] bool captured() const noexcept {
    return state_ == InputState::captured;
  }

  [[nodiscard]] bool capture_intended() const noexcept {
    return state_ == InputState::captured || state_ == InputState::recovering ||
           (state_ == InputState::arming && activation_released_);
  }

  [[nodiscard]] bool special_active() const noexcept {
  ...
  [[nodiscard]] TextMappingResult start_text(
      std::string_view text, Clock::time_point now = Clock::now());
  ...
  [[nodiscard]] InputPointerSnapshot pointer_snapshot() const noexcept {
    return pointer_;
  }
 ...
 private:
  ...
  [[nodiscard]] std::pair<std::uint16_t, std::uint16_t> absolute(
