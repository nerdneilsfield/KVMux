...
Entry printable(const unsigned char c) {
  if (c >= 'a' && c <= 'z')
    return {static_cast<std::uint8_t>(0x04 + c - 'a'), false};
  if (c >= 'A' && c <= 'Z')
    return {static_cast<std::uint8_t>(0x04 + c - 'A'), true};
  if (c >= '1' && c <= '9')
    return {static_cast<std::uint8_t>(0x1e + c - '1'), false};
...
TextFilterResult filter_us_ascii_text(const std::string_view text) {
  ...
  for (std::size_t i = 0; i < text.size(); ++i) {
    ...
    if (c == '\t' || c == '\n' || (c >= 0x20 && c <= 0x7e))
      result.text.push_back(static_cast<char>(c));
    else
      ++result.removed;
  }
...
TextMappingResult map_us_ascii_text(
    const std::string_view text, std::vector<std::uint8_t>* normalized_text) {
  ...
  for (std::size_t i = 0; i < text.size(); ++i) {
    const auto c = static_cast<unsigned char>(text[i]);
    if (c >= 0x80) {
      result.error = TextPasteError::non_ascii;
      return result;
    }
    if (c == '\r') {
      if (i + 1 >= text.size() || text[i + 1] != '\n') {
        result.error = TextPasteError::bare_carriage_return;
        return result;
      }
      ++i;
    ...
    if (!(c == '\t' || c == '\n' || (c >= 0x20 && c <= 0x7e))) {
      result.error = TextPasteError::unsupported;
      return result;
    }
  ...
  if (normalized.empty()) {
    result.error = TextPasteError::empty;
    return result;
  }
  if (normalized.size() > 65536) {
    result.error = TextPasteError::too_long;
    return result;
  }
  if (normalized_text) *normalized_text = normalized;
  result.gestures.reserve(normalized.size());
  for (const auto c : normalized) {
    Entry entry = c == '\t'   ? Entry{0x2b, false}
                  : c == '\n' ? Entry{0x28, false}
                              : printable(c);
    if (entry.usage == 0) {
      result.error = TextPasteError::unsupported;
