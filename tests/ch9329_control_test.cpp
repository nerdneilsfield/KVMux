...
int main() {
  ...
  require(
      encoded == std::vector<std::uint8_t>({0x57, 0xab, 0x00, 0x02, 0x08, 0x02,
                                            0x00, 0x04, 0, 0, 0, 0, 0, 0x12}),
      "keyboard vector");
  ...
  require(decoded.size() == 1 && decoded.front().address == 3 &&
              decoded.front().data[2] == 0xff &&
              decoded.front().data[3] == 0x0f &&
              decoded.front().data[6] == 0xff,
          "fragmented absolute frame");
  ...
  require(keyboard.press(0xe4) && keyboard.modifiers() == 0x10,
          "right control");
  ...
  require(queue.size() == 0 && queue.release_pending() &&
              queue.epoch() == epoch + 1,
