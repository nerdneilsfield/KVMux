...
class Decoder final : public VideoDecoder {
 public:
  ...
  CodecResult configure(const CodecConfig&) override {
    if (value_ == CodecBackend::videotoolbox) ++hardware_configures;
    return {};
  }
  CodecResult submit(const EncodedAccessUnit&) override {
    if (value_ == CodecBackend::videotoolbox)
      return {CodecStatus::unsupported,
              "injected first IDR hardware rejection"};
    ++software_submits;
    return {};
  }
  ...
  CodecDiagnostic diagnostic() const override {
    return {value_, false, false, "test decoder"};
  }
...
void require(bool value) {
  if (!value) throw std::runtime_error("startup fallback check failed");
}
}  // namespace
namespace kvmux {
...
std::unique_ptr<VideoEncoder> create_ffmpeg_encoder(CodecBackend,
                                                    std::string& error) {
  error = "unused test encoder";
  return {};
}
}  // namespace kvmux
int main() {
  ...
  require(decoder->diagnostic().detail.find("injected first IDR") !=
          std::string::npos);
