...
int main() {
    ...
    for (std::size_t n = 0; n < 58; ++n) assert(!relay::decode_annex_b(std::span(hevc).first(n), VideoCodec::hevc));
    for (const auto offset : {32U,36U,40U,44U}) {
        auto bad = hevc; std::fill_n(bad.begin() + offset, 4, 0);
        assert(!relay::decode_annex_b(bad, VideoCodec::hevc)); // zero dimensions or SAR
    }
    for (const auto offset : {32U,36U,40U,44U,48U,49U,50U,51U,52U,53U,54U}) {
        auto bad = hevc; bad[offset] = 255; assert(!relay::decode_annex_b(bad, VideoCodec::hevc));
    }
    for (const auto offset : {49U,50U,51U}) {
        auto bad = hevc; bad[offset] = 3; assert(!relay::decode_annex_b(bad, VideoCodec::hevc)); // reserved color codes
    }
    ...
    assert(decoded_h264 && decoded_h264->codec == VideoCodec::h264 && decoded_h264->bytes == au.bytes);
    assert(!relay::decode_annex_b(h264, VideoCodec::mjpeg));
    au.codec = VideoCodec::hevc;
    auto bad = hevc; bad[58] = 1; assert(!relay::decode_annex_b(bad, VideoCodec::hevc)); // not Annex B
    bad = hevc; bad.pop_back(); assert(!relay::decode_annex_b(bad, VideoCodec::hevc));
    bad = hevc; bad.push_back(0); assert(!relay::decode_annex_b(bad, VideoCodec::hevc));
    ...
    au.bytes.push_back(0); assert(relay::encode_annex_b(au).empty());
