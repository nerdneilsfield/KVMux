...
class FakeSink final : public kvmux::ControlSink {
 public:
  kvmux::SubmitResult submit(kvmux::ControlEvent event) override {
    ...
    if (auto_complete)
      snapshot_value.completed_ordinary_sequence = events.back().sequence;
    return result;
  }
  void release_all() noexcept override {
    ++releases;
    snapshot_value.release_confirmed = false;
  }
  ...
  kvmux::SubmitResult start_ascii_paste(kvmux::AsciiPasteJob job) override {
    ...
    paste_value = {kvmux::AsciiPasteState::active, paste_job.gestures.size(),
                   0};
    return result;
  }
  void cancel_ascii_paste() noexcept override {
    ++paste_cancels;
    if (paste_value.active())
      paste_value.state = kvmux::AsciiPasteState::completed;
  }
  kvmux::SubmitResult start_ascii_paste_text(
      std::vector<std::uint8_t> text) override {
    ++paste_text_starts;
    paste_text = std::move(text);
    paste_value = {kvmux::AsciiPasteState::active, paste_text.size(), 0};
    return result;
  }
  kvmux::AsciiPasteSnapshot ascii_paste_snapshot() const override {
    return paste_value;
  }
...
void capture(kvmux::InputRouter& router, FakeSink& sink, double x = 50,
             double y = 50) {
  router.handle(
      {kvmux::InputButton{kvmux::InputMouseButton::left, true, x, y}});
  require(router.state() == kvmux::InputState::arming,
          "activation starts arming");
  require(sink.events.empty(), "activation down is swallowed");
  router.handle(
      {kvmux::InputButton{kvmux::InputMouseButton::left, false, x, y}});
  router.tick();
...
int main() {
...

  {
    ...
    click();  // Stale video, including a texture retained after capture
              // failure.
...

  {
    ...
    require(router.state() == InputState::releasing && sink.events.empty() &&
                sink.releases == 1,
            "Host key releases without leaking");
    router.handle({InputKey{0xe4, false, false}});
    sink.snapshot_value.release_confirmed = true;
    router.tick();
    require(router.state() == InputState::preview,
            "confirmed release returns preview");
  }

  {
    ...
    require(sink.events.size() == 2 &&
                std::get<KeyEdge>(sink.events[0].payload).pressed &&
...

  {
    ...
    require(sink.events.empty(),
            "pre-held Caps Lock and activation send no keyboard input");
    ...
    auto report =
        ch9329::keyboard_report(0, keyboard.modifiers(), keyboard.keys());
    require(
        report.data == std::vector<std::uint8_t>{0x02, 0, 0x04, 0, 0, 0, 0, 0},
        "Shift is modifier bit, not Caps Lock usage");
    keyboard.clear();
    report = ch9329::keyboard_report(0, keyboard.modifiers(), keyboard.keys());
    require(
        report.data == std::vector<std::uint8_t>(8, 0),
...

  {
    ...
    require(full.x == 0 && full.y == 0 && full.width == 1920 &&
                full.height == 1080 && wide.x == 0 && wide.width == 1920,
            "full frame and matching aspect unchanged");
    require(desktop.x == 96 && desktop.y == 0 && desktop.width == 1728 &&
                desktop.height == 1080,
            "16:10 desktop excludes embedded side bars");
    ...
    require(alternate.x == 20 && alternate.y == 215 && alternate.width == 800 &&
                alternate.height == 450,
            "alternate capture and offset layout use actual display fit");
    FakeSink sink;
    InputRouter router(sink);
    ...
    require(router.state() == InputState::preview,
            "embedded bar cannot activate control");
    capture(router, sink, 960, 540);
    router.handle({InputPointerMotion{960, 540}});
    require(router.pointer_snapshot().submitted_absolute ==
                std::pair<std::uint16_t, std::uint16_t>{2048, 2048},
            "target center maps to HID center");
    require(router.pointer_snapshot().video_local ==
                std::pair<double, double>{864, 540},
    ...
    require(!release.pressed && release.x == 1,
            "drag release in embedded bar clamps to edge");
    router.release();
    router.set_video_rect(full);
    require(!router.pointer_snapshot().video_local,
            "aspect change clears old event geometry");
  }

  {
    ...
    router.handle(
        {InputPointerMotion{wide.x + wide.width, wide.y + wide.height}});
...

  {
    ...
    for (const auto& event : sink.events) {
      ...
      require(motion.dx >= -127 && motion.dx <= 127 && motion.dy >= -127 &&
                  motion.dy <= 127,
    ...
    require(x == 301 && y == -261,
            "fractional and split relative totals conserved");
    ...
    for (const auto source : {std::pair{1920, 1080}, std::pair{1280, 720},
      ...
      for (const double dpi : {1.0, 1.5, 2.0}) {
        ...
        for (const bool framebuffer_coordinates : {false, true}) {
          ...
          const auto motion =
              std::get<AbsoluteMotion>(sink.events.back().payload);
          ...
          const auto wheel =
              std::get<VerticalWheel>(sink.events.back().payload);
...

  {
    ...
    for (const auto rect :
         {Rect{21, 30, 200, 100}, Rect{21, 31, 200, 100},
    ...
    require(!pointer.video_local && !pointer.submitted_absolute &&
                !pointer.submitted_relative,
            "rejected submission clears diagnostics instead of reporting "
  ...
  }
  {
    ...
    capture(router, sink);
    router.handle({InputRelativeMotion{0.4, 0.4}});
    require(!router.pointer_snapshot().submitted_relative,
            "fractional unsent delta stays absent");
    ...
    require(router.pointer_snapshot().submitted_relative == std::pair{2, -2} &&
            ...
            "relative diagnostics report accepted integral delta, not absolute "
            "coordinates");
    router.set_video_fresh(false);
    require(!router.pointer_snapshot().submitted_relative,
            "stale video clears diagnostics");
    router.set_video_fresh(true);
    router.handle({InputRelativeMotion{1, 1}});
    router.release();
    require(!router.pointer_snapshot().submitted_relative,
            "release clears diagnostics");
  }

  {
    ...
    require(progress.active && progress.planned_gestures == 2 &&
                progress.scheduled_gestures == 1,
    ...
    require(!router.text_active() && !progress.active &&
                progress.scheduled_gestures == 2,
            "terminal serial snapshot ends local paste without text edges");
    require(static_cast<bool>(
                router.start_text("ab", start + std::chrono::milliseconds(3))),
            "new local paste starts");
    router.handle({InputKey{0x04, true, false}});
    require(
        !router.text_active() && sink.paste_cancels >= 1 && sink.releases > 0,
        "physical key cancels local composite paste before release");
  }

  {
    // A relay must apply the empty-state barrier before it accepts synthetic
    // edges.
    FakeSink sink;
    ...
    router.tick(start + std::chrono::milliseconds(1));
    require(sink.events.empty(),
            "text waits for the applied synchronization ACK");
    ...
    router.tick(start + std::chrono::milliseconds(2));
    require(sink.paste_text_starts == 1 &&
                sink.paste_text == std::vector<std::uint8_t>{'A'},
            "relay text uploads normalized bytes after the synchronization "
            "barrier");
    router.cancel_text();
...

  {
    ...
    router.set_video_fresh(true);
    const auto start = InputRouter::Clock::now();
    require(static_cast<bool>(router.start_text("A", start)),
            "remote transaction starts");
    const auto& sync = sink.syncs.back();
    sink.snapshot_value.applied = {true, sync.epoch, sync.intent_generation,
                                   sync.revision, sync.state};
    router.tick(start + std::chrono::milliseconds(1));
    require(sink.paste_text_starts == 1,
            "remote transaction passed its barrier");
    for ([[maybe_unused]] const auto phase :
         {"uploading", "complete", "authorizing", "authorized",
          "commit pending", "executing"}) {
...

  {
    ...
    require(mapped && mapped.gestures.size() == text.size(),
            "mapper accepts the relay transaction limit");
