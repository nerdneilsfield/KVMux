...
std::vector<kvmux::EncodedAccessUnit> read_units(const char* path) {
  ...
  auto* context =
      avcodec_alloc_context3(avcodec_find_decoder(AV_CODEC_ID_HEVC));
  ...
  auto parse = [&](const std::uint8_t* data, int count) {
    ...
    int packet_size = 0;
    const int used =
        av_parser_parse2(parser, context, &packet, &packet_size, data, count,
    ...
    if (packet_size) {
      ...
      au.height = 1080;
      au.generation = 7;
      au.capture_sequence = 100 + units.size();
      au.pts_ns = static_cast<std::int64_t>(units.size()) * 16'666'667;
  ...
  std::size_t offset = 0;
  while (offset < size)
    offset += parse(bytes.data() + offset, static_cast<int>(size - offset));
  parse(nullptr, 0);
...
struct RawCapture final : CaptureSource {
  ...
  CaptureSnapshot snapshot() const override {
    ...
    value.state = CaptureState::streaming;
    value.actual_mode = {
        "synthetic",       1920,  1080, {50, 1}, PixelFormat::yuy2,
        PixelFormat::yuy2, "YUY2"};
    return value;
  }
  std::optional<CaptureSample> take_latest_sample() override {
    const auto now = std::chrono::steady_clock::now();
    if (now - previous < 20ms) return {};
    previous = now;
    const std::vector<std::uint8_t> pixels(1920 * 1080 * 2, 128);
    const PlaneLayout plane{0, 3840, 3840, 1080};
    return CaptureSample::make_raw(7, (++sequence) * 10, now, 1920, 1080,
                                   PixelFormat::yuy2, {&plane, 1}, pixels);
  }
};
...
struct FixtureEncoder final : VideoEncoder {
  ...
  bool refresh{};
  explicit FixtureEncoder(
      std::shared_ptr<const std::vector<EncodedAccessUnit>> value)
      : units(std::move(value)) {}
  ...
  CodecResult poll(EncodedAccessUnit& unit) override {
    ...
    unit.generation = config.generation;
    unit.encoded_sequence = ++sequence;
    unit.capture_sequence = input->capture_sequence;
    unit.arrival = input->arrival;
    unit.pts_ns = input->pts_ns;
    input.reset();
    return {};
  }
  ...
  CodecDiagnostic diagnostic() const override {
    return {backend(), false, false, "Synthetic pre-encoded HEVC"};
  }
};
void hevc_test(const char* path) {
  auto units =
      std::make_shared<const std::vector<EncodedAccessUnit>>(read_units(path));
  assert(units->size() == 60);
  RawCapture capture;
  FakeSerial serial;
  Ch9329ControlSink sink(serial.io());
  sink.connect("fake", 115200);
  assert(until([&] { return sink.snapshot().release_confirmed; }));
  kvmux::relay::RelayServer server(
      capture, sink, [units](CodecBackend, std::string&) {
        return std::make_unique<FixtureEncoder>(units);
      });
  kvmux::relay::ServerOptions options;
  options.bind_address = "127.0.0.1";
  options.control_port = options.video_port = 0;
  ...
  assert(server.start(options, error));
  {
    ...
    blocked_options.control_port = server.control_port();
    blocked_options.video_port = server.video_port();
    GuiFixture blocked(blocked_options);
    assert(blocked.wait([&] {
      return server.snapshot().last_reason ==
             kvmux::relay::MediaReason::frame_exceeds_rate_budget;
    }));
    blocked.run_for(200ms);
    assert(!blocked.session.snapshot().video_fresh);
    assert(server.snapshot().pacer.rejected_frames > 0);
    assert(std::string(kvmux::relay::media_reason_name(
               server.snapshot().last_reason)) == "frame_exceeds_rate_budget");
  }
  server.stop();
  options.transport_bytes_per_second = 12'000'000;
  assert(server.start(
      options,
      error));  // New generation and fresh IDR, no in-session configure.

  kvmux::relay::ClientOptions client_options;
  client_options.control_port = server.control_port();
  client_options.video_port = server.video_port();
  UdpProxy proxy(client_options);
  client_options = proxy.options();
  client_options.decoder_backend = CodecBackend::ffmpeg_software;
  GuiFixture gui(client_options);
  gui.run_for(250ms);
  assert(proxy.challenges >=
         3);  // Slow configure did not stop the network owner.
  gui.activate();
  assert(gui.client->video_snapshot().codec == VideoCodec::hevc);
  assert(proxy.last_presented > 0 &&
         proxy.last_presented < capture.sequence * 10);
  ...
  assert(gui.wait(
      [&] { return gui.client->video_snapshot().recoveries > recovered; }));
  assert(gui.wait([&] {
  ...
  gui.run_for(600ms);
  const auto admitted =
      server.snapshot().source_admissions - slowed.source_admissions;
  assert(admitted > 0 &&
         admitted <= static_cast<std::uint64_t>(
                         600000 / slowed.admission_interval.count()) +
                         2);
  // A forged/stale GUI ID must not move the proof high-water to that value.
  gui.client->video_presented(gui.client->capture_snapshot().generation,
                              99999999);
  gui.run_for(80ms);
  assert(proxy.last_presented < 99999999);
...
void chunked_ascii_paste_loopback_test(const char* jpeg) {
  ...
  assert(gui.wait([&] {
    ...
    return state.input_state == InputState::preview &&
           state.control.release_confirmed && state.video_fresh &&
           state.capture.state == CaptureState::streaming &&
           state.capture.actual_mode.width != 0 &&
           state.capture.actual_mode.height != 0;
  }));
  ...
  assert(gui.wait(
      [&] {
        ...
        executing = executing ||
                    paste.state == kvmux::relay::PasteUploadState::executing;
        // Authorization is after upload but before the server starts serial
        // I/O.
        if (paste.state == kvmux::relay::PasteUploadState::preparing ||
            paste.state == kvmux::relay::PasteUploadState::executing) {
          active_before_serial =
              active_before_serial || gui.session.snapshot().text_paste_active;
        }
  ...
  const auto paste = gui.client->ascii_paste_text_snapshot();
  assert(executing);
  assert(paste.total_bytes == text_bytes &&
         paste.accepted_bytes == text_bytes &&
         paste.completed_bytes == text_bytes &&
         paste.reason == kvmux::relay::wire::PasteStatusReason::none);
  ...
  assert(gui.wait([&] {
    const auto state = gui.session.snapshot();
    return !state.text_paste_active &&
           state.input_state == InputState::preview &&
  ...
  }));
  {
    ...
    const auto keyboard_reports_after = static_cast<std::size_t>(std::count_if(
        relay.serial.received.begin(), relay.serial.received.end(),
        [](const auto& frame) { return frame.command == 0x02U; }));
    // 961 bytes cross the normalized 960-byte upload chunk boundary. The
    // temporary remote intent contributes one synchronized empty report and one
    // release report.
    assert(keyboard_reports_after ==
           keyboard_reports_before + 2 + text_bytes * 2);
  }
  // The relay-private authorization revision must not collide with the GUI
  // revision stream.
...
void paste_keepalive_survives_blackholed_server_status_test(const char* jpeg) {
  RelayFixture relay(jpeg);
  {
    std::lock_guard lock(relay.serial.mutex);
    relay.serial.write_limit = 4096;
    relay.serial.hold_keyboard_ack = true;
  }
  ...
  assert(gui.wait([&] {
    return gui.client->ascii_paste_text_snapshot().state ==
           kvmux::relay::PasteUploadState::executing;
  }));
  ...
  proxy.drop_server_kcp = false;
  {
    ...
    relay.serial.incoming.insert(relay.serial.incoming.end(),
                                 relay.serial.held_ack.begin(),
                                 relay.serial.held_ack.end());
    relay.serial.held_ack.clear();
  }
  assert(gui.wait(
      [&] {
        return gui.client->ascii_paste_text_snapshot().state ==
               kvmux::relay::PasteUploadState::completed;
      },
      5000ms));
  gui.session.release_control();
}
...
void first_frame_activation_test(const char* jpeg) {
  ...
  gui.after_session_tick = [&] {
    ...
    assert(until(
        [&] { return gui.session.snapshot().video.processed_frames > 0; }));
    auto frame = gui.session.take_latest_frame();
    assert(frame);
    ...
    assert(until([&] {
      ...
      return state.control.state == ControlConnectionState::ready &&
             state.control.target_usb_ready &&
             state.control.release_confirmed && state.video_fresh;
    }));
  };
  gui.activate();
...
void input_recovery_test(const char* jpeg) {
  ...
  {
    std::lock_guard lock(relay.serial.mutex);
    relay.serial.hold_mouse_ack = true;
  }
  gui.session.handle_input({InputButton{InputMouseButton::left, true, 50, 50}});
  gui.session.handle_input(
      {InputButton{InputMouseButton::left, false, 50, 50}});
  assert(gui.wait([&] {
    std::lock_guard lock(relay.serial.mutex);
    return !relay.serial.held_ack.empty();
  }));
  ...
  assert(gui.wait([&] {
    std::lock_guard lock(relay.serial.mutex);
    return std::any_of(relay.serial.received.begin(),
                       relay.serial.received.end(), [](const auto& frame) {
                         return frame.command == 2 &&
                                std::find(frame.data.begin(), frame.data.end(),
                                          4) != frame.data.end();
                       });
  }));
  const auto traffic = gui.client->traffic_snapshot();
  assert(traffic.video_received_bytes && traffic.control_received_bytes &&
         traffic.control_sent_bytes);
  gui.session.handle_input({InputKey{0x04, false, false}});
  relay.capture.stalled = true;
  gui.run_for(650ms);
  assert(gui.session.snapshot().input_state == InputState::recovering);
  ...
  assert(gui.session.send_special(SpecialKeys::host_key));
  gui.run_for(300ms);
  assert(gui.session.snapshot().input_state == InputState::preview);
  {
    std::lock_guard lock(relay.serial.mutex);
    assert(std::any_of(relay.serial.received.begin(),
                       relay.serial.received.end(), [](const auto& report) {
                         return report.command == 2 &&
                                report.data.size() >= 2 &&
                                (report.data[0] & 0x10);
                       }));
  }
  ...
  gui.activate();
  assert(relay.sink.snapshot().applied.known &&
         relay.sink.snapshot().applied.state.mode == MouseMode::relative);
  gui.session.release_control();
}
...
void run_concurrent_network_front_start(const char* jpeg, const char*) {
  concurrent_network_front_start_test(jpeg);
}
void run_first_frame_activation(const char* jpeg, const char*) {
  first_frame_activation_test(jpeg);
}
void run_chunked_ascii_paste_loopback(const char* jpeg, const char*) {
  chunked_ascii_paste_loopback_test(jpeg);
}
void run_paste_keepalive_blackholed_status(const char* jpeg, const char*) {
  paste_keepalive_survives_blackholed_server_status_test(jpeg);
}
void run_hevc(const char*, const char* hevc) { hevc_test(hevc); }
void run_input_recovery(const char* jpeg, const char*) {
  input_recovery_test(jpeg);
}

int main(int argc, char** argv) {
  assert(argc >= 3);
  const std::array cases{
      TestCase{"concurrent_network_front_start",
               run_concurrent_network_front_start},
      TestCase{"first_frame_activation", run_first_frame_activation},
      TestCase{"chunked_ascii_paste_loopback",
               run_chunked_ascii_paste_loopback},
      TestCase{"paste_keepalive_blackholed_status",
               run_paste_keepalive_blackholed_status},
      TestCase{"hevc", run_hevc},
      TestCase{"input_recovery", run_input_recovery},
  };
  std::optional<std::string_view> requested;
  if (argc == 5 && std::string_view(argv[3]) == "--case")
    requested = argv[4];
  else if (argc != 3) {
