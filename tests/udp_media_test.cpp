...
MediaFrame make_frame(VideoCodec codec, std::uint64_t seq, std::size_t compressed, bool idr = false) {
    ...
    assert(compressed >= 7);
    data[0] = 0; data[1] = 0; data[2] = 0; data[3] = 1; data[4] = idr ? 0x26 : 0x02; data[5] = 1;
...
struct Events {
    ...
    void add(std::vector<MediaEvent> batch) {
        ...
        for (auto& e : batch) {
            ...
            if (e.kind == MediaEvent::Kind::frame) { bytes += e.frame->bytes.size(); frames.push_back(std::move(*e.frame)); marker = e.recovery_marker; }
...
Events send(MediaReceiver& receiver, const MediaFrame& f, MediaTime now = epoch,
            std::vector<std::size_t> missing = {}, bool reverse = false, bool duplicate = false) {
    Events result;
    const auto n = media_packet_count(f);
    for (std::size_t i = 0; i < n; ++i) {
        const auto ordinal = reverse ? n - 1 - i : i;
        if (std::find(missing.begin(), missing.end(), ordinal) != missing.end()) continue;
        const auto packet = media_packet(f, ordinal); assert(packet && packet->size() + 32 <= 1200);
        result.add(receiver.input(*packet, now));
        if (duplicate) result.add(receiver.input(*packet, now));
        assert(receiver.stats().charged_bytes <= kMediaAllocationLimit && receiver.stats().resident_frames <= 8);
    }
    return result;
}
void roundtrips() {
    for (auto codec : {VideoCodec::mjpeg, VideoCodec::hevc, VideoCodec::h264}) {
        ...
        auto bad = send(failed, f, epoch, {0,1}); assert(bad.frames.empty());
        bad.add(failed.poll(epoch + 150ms)); assert(bad.frames.empty() && bad.losses == 1);
...
void malformed_and_bounds() {
    ...
    for (auto offset : {0U,1U,2U,3U,4U,20U,24U,26U,28U,30U,32U}) {
        auto bad = p; bad[offset] = 255;
        MediaReceiver receiver(VideoCodec::mjpeg, 7);
        receiver.input(bad, epoch); assert(receiver.stats().resident_frames == 0);
    }
    for (auto size : {std::size_t(0), std::size_t(39), std::size_t(1169)}) {
        auto bad = p; bad.resize(size);
        MediaReceiver receiver(VideoCodec::mjpeg, 7); receiver.input(bad, epoch);
        assert(receiver.stats().resident_frames == 0);
    }
    MediaReceiver conflict(VideoCodec::mjpeg, 7);
    conflict.input(p, epoch); auto changed = p; changed[40] ^= 1;
    Events result; result.add(conflict.input(changed, epoch)); assert(result.losses == 1);
    assert(send(conflict, f).frames.empty());
    MediaReceiver aged(VideoCodec::mjpeg, 7);
    aged.input(p, epoch); aged.input(p, epoch + 149ms); aged.poll(epoch + 150ms);
    ...
    for (std::uint64_t seq = 1; seq <= 20; ++seq) {
        ...
        flood.input(*media_packet(candidate, 0), epoch + std::chrono::milliseconds(seq));
        assert(flood.stats().resident_frames <= 8 && flood.stats().charged_bytes <= kMediaAllocationLimit);
    }
    ...
    huge.sequence = 2; bytes.input(*media_packet(huge, 0), epoch + 1ms);
    assert(bytes.stats().resident_frames == 1 && bytes.stats().capacity_losses == 1);
    ...
    auto body_result = send(body, invalid); assert(body_result.frames.empty() && body_result.losses == 1);
...
void hevc_order() {
    ...
    auto first = send(r, idr); assert(first.frames.size() == 1 && first.resets == 1);
    const auto reset = std::find(first.order.begin(), first.order.end(), MediaEvent::Kind::reset);
    const auto frame = std::find(first.order.begin(), first.order.end(), MediaEvent::Kind::frame);
    ...
    r.poll(epoch + 40ms); assert(!r.stats().waiting_idr);
    Events gap; gap.add(r.poll(epoch + 41ms)); assert(gap.resets == 1 && gap.refreshes == 0 && r.stats().waiting_idr);
    ...
    Events refreshes;
    for (int ms = 42; ms <= 350; ++ms) refreshes.add(r.poll(epoch + std::chrono::milliseconds(ms)));
    assert(refreshes.refreshes == 3 && r.stats().waiting_idr);
    auto recovery = send(r, make_frame(VideoCodec::hevc, 10, 20000, true), epoch + 351ms);
    assert(recovery.frames.size() == 1 && recovery.marker > first.marker);
    auto resumed = send(r, make_frame(VideoCodec::hevc, 11, 20000), epoch + 352ms);
    assert(resumed.frames.size() == 1 && resumed.frames[0].sequence == 11);
    assert(send(r, p3, epoch + 353ms).frames.empty());
    Events reset_failure; reset_failure.add(r.recover(MediaReason::decoder_failure, epoch + 354ms));
    assert(reset_failure.resets == 1 && r.stats().waiting_idr);
    MediaReceiver generation(VideoCodec::hevc, 8);
    assert(send(generation, idr).frames.empty() && generation.stats().resident_frames == 0);
    // Reordered P frames flush strictly in order when the missing prefix arrives.
    MediaReceiver ordered(VideoCodec::hevc, 7); send(ordered, idr);
    assert(send(ordered, p3, epoch + 1ms).frames.empty());
    auto pair = send(ordered, p2, epoch + 2ms);
    assert(pair.frames.size() == 2 && pair.frames[0].sequence == 2 && pair.frames[1].sequence == 3);
    MediaReceiver bypass(VideoCodec::hevc, 7); send(bypass, idr); send(bypass, p3, epoch + 1ms);
    auto fresh = send(bypass, make_frame(VideoCodec::hevc, 10, 100, true), epoch + 2ms);
    assert(fresh.frames.size() == 1 && fresh.resets == 1 && fresh.frames[0].sequence == 10);
    ...
    assert(replacement.frames.size() == 1 && replacement.frames[0].sequence == 10);
    assert(replacement.marker > old_chain.marker); // T3 must fence old decoder output.
    assert(std::find(replacement.order.begin(), replacement.order.end(), MediaEvent::Kind::reset) <
           std::find(replacement.order.begin(), replacement.order.end(), MediaEvent::Kind::frame));
    const auto eleven = send(future, make_frame(VideoCodec::hevc, 11, 100), epoch + 3ms);
    ...
    assert(send(future, idr10, epoch + 5ms).frames.empty()); // Delivered watermark cannot rewind.
    // Both a required incomplete AU and explicit ingress failure break references.
    MediaReceiver age(VideoCodec::hevc, 7); send(age, idr);
    age.input(*media_packet(p2, 0), epoch + 1ms);
    Events expired; expired.add(age.poll(epoch + 151ms));
    assert(expired.resets == 1 && age.stats().age_losses == 1);
    Events overflow; overflow.add(age.recover(MediaReason::ingress_overflow, epoch + 152ms));
    assert(overflow.resets == 1 && age.stats().waiting_idr);
    MediaReceiver capacity(VideoCodec::hevc, 7); send(capacity, idr);
...
void pacing() {
    ...
    for (int ms = 0; ms <= 1000; ++ms) {
        ...
        if (ms % 5 == 0) { if (latest) ++replaced; latest = make_frame(VideoCodec::mjpeg, ++sequence, 4000); latest->first_arrival = now; }
        pacer.poll(now);
        if (latest && pacer.can_start(now)) { assert(pacer.submit(std::move(*latest), now).accepted); latest.reset(); }
        for (int pull = 0; pull < 2; ++pull) {
            auto packet = pacer.next_datagram(now); if (!packet) break;
            bytes += packet->size() + 32;
            for (const auto& e : receiver.input(*packet, now)) if (e.kind == MediaEvent::Kind::frame) ++delivered;
        }
    ...
    assert(!budget.accepted && budget.needs_idr && budget.reason == MediaReason::frame_exceeds_rate_budget);
    ...
    auto expired_pending = hevc.discard(MediaReason::sender_deadline); assert(expired_pending.needs_idr);
    ...
    assert(stale.submit(old_source, epoch + 250ms).reason == MediaReason::source_stale);
    auto deadline = hevc.next_deadline(epoch); assert(deadline && *deadline <= epoch + 100ms);
    ...
    auto f = make_frame(VideoCodec::mjpeg, 1, 10000); f.first_arrival = epoch + 10s;
    ...
    for (auto blackout : {100ms,300ms,800ms,2000ms}) for (auto codec : {VideoCodec::mjpeg, VideoCodec::hevc, VideoCodec::h264}) {
        ...
        for (int ms = 0; ms < 100; ++ms) {
            ...
            for (int i = 0; i < 2; ++i) if (auto packet = paused.next_datagram(now)) result.add(recovered.input(*packet, now));
        }
        assert(result.frames.size() == 1 && result.frames[0].sequence == 10);
    }
    std::cout << "pacer: " << bytes << " charged bytes/1000ms, " << replaced << " pre-encode source replacements, " << delivered << " delivered frames\n";
}
void random_loss() {
    ...
    for (std::uint64_t seq = 1; seq <= 1000; ++seq) {
        ...
        for (std::size_t i = 0; i < media_packet_count(f); ++i) {
            ...
            for (auto& e : a.input(p, epoch)) if (e.kind == MediaEvent::Kind::frame) ++fec;
            if (p[2] == 0) for (auto& e : b.input(p, epoch)) if (e.kind == MediaEvent::Kind::frame) ++plain;
        }
    }
    assert(fec > plain + 250 && fec > 900);
    std::cout << "seeded 1% independent loss/1000 frames: XOR=" << fec << ", no parity=" << plain << "\n";
}
void native_udp() {
    std::string error;
    auto a = udp::Socket::bind("127.0.0.1", 0, error), b = udp::Socket::bind("127.0.0.1", 0, error); assert(a && b);
    auto destination = udp::resolve("127.0.0.1", *b->local_port(), error); assert(destination);
    const auto f = make_frame(VideoCodec::mjpeg, 1, 2000);
    MediaReceiver receiver(VideoCodec::mjpeg, 7); Events result;
    for (std::size_t i = 0; i < media_packet_count(f); ++i) {
        auto p = *media_packet(f, i); p.insert(p.begin(), 32, 0);
        assert(a->send_to(*destination, p) == udp::SendStatus::sent);
        auto received = b->receive(100ms); assert(received.status == udp::ReceiveStatus::datagram);
        result.add(receiver.input(std::span(received.datagram.bytes).subspan(32), epoch));
    }
    assert(result.frames.size() == 1 && result.frames[0].bytes == f.bytes);
}
} // namespace
int main() {
    roundtrips(); malformed_and_bounds(); hevc_order(); pacing(); random_loss(); native_udp();
    std::cout << "UDP media boundaries, ordering, burst recovery, blackouts and native roundtrip passed\n";
}
