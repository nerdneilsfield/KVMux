...
inline bool until(const std::function<bool()>& condition,
...
struct RelayFixture {
  ...
  explicit RelayFixture(const char* jpeg, std::uint64_t rate = 12000000) {
    capture.jpeg = fixture_bytes(jpeg);
    ...
    options.transport_bytes_per_second = rate;
    std::string error;
    assert(server.start(options, error));
  }
  kvmux::relay::ClientOptions options() const {
    kvmux::relay::ClientOptions options;
    options.control_port = server.control_port();
    options.video_port = server.video_port();
    return options;
  }
};
// Native two-front-port proxy. A single backend socket preserves the client's
// endpoint identity across both server sockets. Impairment drops whole
// datagrams.
struct UdpProxy {
  ...
  std::atomic<std::uint64_t> session_id{}, challenges{}, proofs{}, media{},
      last_presented{}, drop_media_sequence{}, last_media_sequence{};
  std::thread worker;
  explicit UdpProxy(kvmux::relay::ClientOptions options) {
    ...
    server_control =
        *kvmux::udp::resolve("127.0.0.1", options.control_port, error);
    ...
    worker = std::thread([this] {
      while (!done) {
        for (unsigned i = 0; i < 32; ++i) {
          ...
          const auto envelope =
              kvmux::relay::wire::decode_envelope(packet.datagram.bytes);
          if (envelope &&
              envelope->kind == kvmux::relay::wire::EnvelopeKind::proof) {
            ++proofs;
            last_presented = std::get<kvmux::relay::wire::Proof>(
                                 *kvmux::relay::wire::decode_raw(
                                     envelope->kind, envelope->body))
                                 .presented_sequence;
          }
          if (!blackout)
            (void)backend.send_to(server_control, packet.datagram.bytes);
        }
        for (unsigned i = 0; i < 64; ++i) {
          ...
          if (envelope->kind == Kind::ready) {
            session_id = envelope->tuple.session;
            if (drop_ready) {
              --drop_ready;
              continue;
            }
          }
          if (envelope->kind == Kind::welcome && drop_welcome) {
            --drop_welcome;
            continue;
          }
          ...
          if (envelope->kind == Kind::media) {
            ...
            for (unsigned n = 12; n < 20; ++n)
              sequence = (sequence << 8U) | envelope->body[n];
            last_media_sequence = sequence;
            if (sequence == drop_media_sequence) continue;
          }
          if (blackout || (drop_server_kcp && envelope->kind == Kind::kcp))
            continue;
          if (packet.datagram.source == server_control)
            (void)control.send_to(*client, packet.datagram.bytes);
          else if (packet.datagram.source == server_video) {
            if (wrong_tuple_only) packet.datagram.bytes[15] ^= 0x01;
            if (wrong_source_only)
              (void)control.send_to(*client, packet.datagram.bytes);
            else
              (void)video.send_to(*client, packet.datagram.bytes);
          }
        }
      }
    });
  ...
  kvmux::relay::ClientOptions options() const {
    kvmux::relay::ClientOptions options;
    options.control_port = *control.local_port();
    options.video_port = *video.local_port();
    return options;
  }
};
struct GatedNetworkCapture final : CaptureSource {
  ...
  explicit GatedNetworkCapture(std::shared_ptr<kvmux::relay::RelayClient> value)
  ...
  std::optional<CaptureSample> take_latest_sample() override {
    return hold ? std::nullopt : client->take_sample();
  }
};
struct GuiFixture {
  std::shared_ptr<kvmux::relay::RelayClient> client;
  ...
  explicit GuiFixture(kvmux::relay::ClientOptions options,
    ...
    assert(session.select_capture(device, mode));
  ...
  void activate() {
    ...
    assert(until([&] {
      ...
      const bool displayed =
          presented_generation == before.capture.generation &&
    ...
    const bool captured = wait(
        [&] { return session.snapshot().input_state == InputState::captured; });
    if (!captured) {
      const auto state = session.snapshot();
      std::fprintf(stderr,
                   "activation input=%d control=%d epoch=%llu applied=%d "
                   "rev=%llu fresh=%d error=%s\n",
                   int(state.input_state), int(state.control.state),
                   (unsigned long long)state.control.epoch,
                   state.control.applied.known,
                   (unsigned long long)state.control.applied.revision,
                   state.video_fresh, state.control.error.c_str());
    }
    assert(captured);
  }
};
