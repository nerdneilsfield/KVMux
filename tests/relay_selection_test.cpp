...
template <class F>
void fails(F call, const std::string& text) {
  try {
    call();
    assert(false);
...
int main() {
  DeviceInfo a;
  a.stable_id = "camera-a";
  a.display_name = "A";
  DeviceInfo b;
  b.stable_id = "camera-b";
  b.display_name = "B";
  ...
  modes.pop_back();
  assert(select_mode(modes) == 3);
  modes.pop_back();
  assert(select_mode(modes) == 2);
  modes.pop_back();
  assert(select_mode(modes) == 1);
  modes.pop_back();
  assert(select_mode(modes) == 0);
  modes.push_back(mode(1920, 1200, {100, 1}));
  assert(select_mode(modes) == 1);
  modes.push_back(mode(640, 480, {200, 1}));
  assert(select_mode(modes) == 1);
  assert(select_mode(modes, 2) == 2);
  modes.push_back(mode(1920, 1080, {5994, 100}));
  assert(select_mode(modes) == 3);
  modes.back().delivered_format = PixelFormat::bgra;
  assert(select_mode(modes) == 1);
  fails([&] { select_mode(modes, 3); }, "MJPEG");
  modes.back().delivered_format = PixelFormat::mjpeg;
  modes.back().device_format = PixelFormat::yuy2;
  assert(select_mode(modes) == 1);
  fails([&] { select_mode(modes, 3); }, "MJPEG");
  ...
  std::vector<SerialPortInfo> ports{
      {"/dev/ttyTHS0", "board", {}, {}},
      {"/dev/ttyUSB0", "other", 0x0403, 0x6001},
  ...
  ports.pop_back();
  fails([&] { select_serial(ports); }, "specify --serial");
