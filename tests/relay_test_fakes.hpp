    ...
    return {
        ...
        [this](std::span<const std::uint8_t> bytes) -> std::ptrdiff_t {
          ...
          for (const auto& request : frames) {
            ...
            auto& destination =
                (hold_mouse_ack &&
                 (request.command == 0x04U || request.command == 0x05U)) ||
                        (hold_keyboard_ack && request.command == 0x02U)
                    ? held_ack
                    : incoming;
            destination.insert(destination.end(), encoded.begin(),
                               encoded.end());
          }
...
struct FakeCapture final : CaptureSource {
  ...
  std::optional<CaptureSample> take_latest_sample() override {
    ...
    if (stalled || now - previous < 20ms) return {};
    previous = now;
    return CaptureSample::make_mjpeg(1, ++sequence, now, 16, 16, jpeg);
  }
  CaptureSnapshot snapshot() const override {
    CaptureSnapshot out;
    out.state = CaptureState::streaming;
    out.actual_mode = {
        "fake", 16, 16, {50, 1}, PixelFormat::mjpeg, PixelFormat::mjpeg,
        "MJPEG"};
    return out;
  }
};
