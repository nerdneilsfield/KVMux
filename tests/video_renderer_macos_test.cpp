...
int reconnect_test(GLuint ui_framebuffer, const char* jpeg_path) {
    FakeSerial serial; kvmux::Ch9329ControlSink hardware(serial.io()); hardware.connect("fake", 57600);
    FakeCapture capture;
    std::ifstream input(jpeg_path, std::ios::binary);
    capture.jpeg.assign(std::istreambuf_iterator<char>(input), {});
    kvmux::relay::RelayServer server(capture, hardware); std::string error;
    ...
    int failures = 0;
    auto begin = [&] { ImGui_ImplOpenGL3_NewFrame(); ImGui_ImplSDL3_NewFrame(); ImGui::NewFrame();
        ImGui::Begin("KVMuxRoot"); ImGui::TextUnformatted("Connect relay"); };
    auto end = [&] {
        if (renderer.texture_id()) ImGui::GetBackgroundDrawList()->AddImage(
            static_cast<ImTextureID>(renderer.texture_id()), ImVec2(0,0), ImVec2(100,100));
        ImGui::End(); ImGui::Render(); glBindFramebuffer(GL_FRAMEBUFFER, ui_framebuffer);
        ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData()); glBindFramebuffer(GL_FRAMEBUFFER, 0);
    };
    for (int cycle = 0; cycle < 5; ++cycle) {
        ...
        if (current && !renderer.upload(*current, kvmux::ColorOverride::automatic)) ++failures;
        begin();
        session.reset(); current.reset(); renderer.destroy();
        auto client = std::make_shared<kvmux::relay::RelayClient>(kvmux::relay::ClientOptions{
        ...
        session = std::make_unique<kvmux::KvmSession>(std::move(source),
            std::make_unique<kvmux::relay::NetworkControlSink>(client));
        (void)session->select_capture(device, mode); end();
        int frames = 0;
        ...
        while (frames < 20 && std::chrono::steady_clock::now() < deadline) {
            ...
            if (current) {
                ...
                if (rgba[0] < 245 || rgba[1] > 10 || rgba[2] > 10 || internal != GL_RGBA8) {
                    std::fprintf(stderr, "reconnect=%d frames=%d RGB=%d,%d,%d internal=%x\n", cycle, frames, rgba[0],rgba[1],rgba[2],internal); ++failures;
                }
            }
            begin(); end();
            if (current) {
    ...
    session.reset(); renderer.destroy(); server.stop(); hardware.disconnect();
    std::printf("actual relay / session / MJPEG / ImGui reconnect: %d failures\n", failures);
    return failures;
}

// Real macOS SDL/OpenGL storage and reconnect regression; no capture hardware needed.
int main(int argc, char** argv) {
    ...
    auto* window = SDL_CreateWindow("KVMux reconnect test", 320, 240, SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN);
    auto context = SDL_GL_CreateContext(window);
    if (!window || !context || !SDL_GL_MakeCurrent(window, context)) return 2;
    if (!gladLoadGL([](const char* name) { return reinterpret_cast<GLADapiproc>(SDL_GL_GetProcAddress(name)); })) return 2;
    ImGui::CreateContext();
